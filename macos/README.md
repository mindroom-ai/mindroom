...
The menu bar always uses a monochrome vector M, automatically tinted by AppKit, with service status in its tooltip and menu.

`Resources/MindRoom.icon` packages the appearance variants, and `Resources/Assets.xcassets` contains the menu bar template.
The build compiles both with `actool`, copies the compiled assets into the app, and merges the generated icon metadata into `Info.plist`.
