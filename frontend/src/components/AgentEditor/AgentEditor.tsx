...
import { AgentMemory } from '@/components/AgentMemory/AgentMemory';
...
export function AgentEditor() {
  ...
  const [editorView, setEditorView] = useState<'details' | 'memory'>('details');
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center gap-2">
        <Button
          size="sm"
          variant={editorView === 'details' ? 'default' : 'outline'}
        ...
        <Button
          size="sm"
          variant={editorView === 'memory' ? 'default' : 'outline'}
          onClick={() => setEditorView('memory')}
        >
          Memory
        </Button>
      </div>
      ...
      {editorView === 'details' ? (
        ...
        <AgentMemory agentId={selectedAgent.id} />
      )}

      {/* Tool Configuration Dialog */}
      {editorView === 'details' && configDialogTool && (
        <ToolConfigDialog
