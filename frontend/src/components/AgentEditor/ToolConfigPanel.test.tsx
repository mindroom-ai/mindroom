...
describe('ToolConfigPanel', () => {
  ...
  it('renders "no settings available" when tool has neither override fields nor config fields', () => {
    ...
    expect(screen.getByText('No per-agent settings available for this tool.')).toBeInTheDocument();
  });
  ...
  it('commits override updates when toggling and editing string[] fields', () => {
    ...
    const pathToggle = screen.getByRole('checkbox', { name: 'Override PATH Prepend' });
  ...
  it('prefers overrideFields over configFields when both are present', () => {
    ...
    render(
      <ToolConfigPanel
        ...
        overrideFields={[
          { name: 'extra_env_passthrough', label: 'Env Passthrough', type: 'string[]' },
        ]}
