...
function cloneOverrides(overrides: ToolOverrides | null | undefined): ToolOverrides {
  if (overrides == null) {
    return {};
  }
  return Object.fromEntries(
    Object.entries(overrides).map(([key, value]) => [key, cloneOverrideValue(value)])
  );
}

function parseToolEntry(entry: ToolEntry): { name: string; overrides: ToolOverrides | null } {
...
function makeStructuredToolEntry(toolName: string, overrides: ToolOverrides): ToolEntry {
...
function sanitizeOverrides(overrides: ToolOverrides | null | undefined): ToolOverrides {
  if (overrides == null) {
    return {};
  }
...
export function cloneToolEntries(rawEntries: ToolEntry[] | null | undefined): ToolEntry[] {
  if (!rawEntries) {
    return [];
  }
...
export function normalizeToolEntries(rawEntries: ToolEntry[] | null | undefined): string[] {
  if (!rawEntries) {
    return [];
  }
...
export function rebuildToolEntries(
  ...
  return toolNames.map(toolName => rawEntriesByName.get(toolName) ?? toolName);
}
...
export function getToolOverrides(
  ...
  const matchingEntry = rawEntries.find(entry => extractToolName(entry) === toolName);
  if (!matchingEntry) {
    return null;
  }
...
export function setToolOverridesInEntries(
  ...
  const mergedOverrides = sanitizeOverrides({ ...existingOverrides, ...(overrides ?? {}) });
  const replacementEntry =
    Object.keys(mergedOverrides).length > 0
      ? makeStructuredToolEntry(toolName, mergedOverrides)
      : toolName;
  const existingIndex = nextEntries.findIndex(entry => extractToolName(entry) === toolName);
