...
const ROOT: JsonSchema = {
  ...
  $defs: {
    ...
    JudgmentConfig: {
      oneOf: [{ $ref: '#/$defs/LLMJudgmentConfig' }, { $ref: '#/$defs/TypeSafeJudgmentConfig' }],
      discriminator: {
...
describe('classifySchemaNode', () => {
  ...
  it('classifies a discriminated union behind a nullable alias', () => {
    const node = classifySchemaNode(ROOT.$defs!.ParticipationConfig.properties!.judgment, ROOT);
    expect(node.kind).toBe('union');
    expect(node.nullable).toBe(true);
    expect(node.discriminator).toBe('provider');
    expect(node.variants.map(variant => variant.label)).toEqual(['LLM', 'Typesafe']);
    expect(node.variants.map(variant => variant.discriminatorValue)).toEqual(['llm', 'typesafe']);
  });
  ...
  it('classifies scalar kinds', () => {
    ...
    const literal = classifySchemaNode({ enum: ['sidecar', 'split'], type: 'string' }, ROOT);
    expect(literal.kind).toBe('enum');
    expect(literal.options).toEqual(['sidecar', 'split']);
    expect(classifySchemaNode({ const: 'llm', type: 'string' }, ROOT).kind).toBe('const');
  });
  ...
  it('classifies maps and freeform objects', () => {
    ...
    expect(classifySchemaNode({ type: 'object', additionalProperties: true }, ROOT).kind).toBe(
      'freeform'
    );
...
describe('initialValue', () => {
  ...
  it('starts objects with only their required fields', () => {
    const node = classifySchemaNode({ $ref: '#/$defs/ParticipationConfig' }, ROOT);
  ...
  it('starts collections empty', () => {
    ...
    expect(
      initialValue(
        classifySchemaNode({ type: 'object', additionalProperties: { type: 'string' } }, ROOT),
        ROOT,
        REFERENCES
      )
    ).toEqual({});
