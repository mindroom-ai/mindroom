...
At most one candidate collection per knowledge base is retained; superseded candidates are deleted, and collections whose ownership cannot be proven from the base identity are preserved and reported rather than removed.
...
- Monitor indexing status (file count vs. indexed count, plus in-progress candidate progress when a build is running)
