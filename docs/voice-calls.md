...
Locally discovered or configured focuses may use private endpoints, while any unpinned or inherited focus requires HTTPS and uses server-side fetch guards that reject private, local, and metadata endpoints before sending the short-lived Matrix OpenID identity token.
