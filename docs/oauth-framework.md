...
Shared-scope reset links use a one-time capability for configured credential managers: the GET is non-mutating, the confirmation POST consumes the reset capability before deleting the scoped credential, and reconnection continues through a fresh single-use connect link that the browser user must redeem under the authenticated shared-scope checks above.
