...
Hosted OAuth entrypoints accept that dynamically registered client only when `MINDROOM_PUBLIC_URL` or `MINDROOM_BASE_URL` produces an exact, unambiguous HTTPS callback on the same publicly routable host as the initiating request and the authorization server confirms that callback in its registration response.
Missing, replaced, insecure, non-public, cross-host, or ambiguous callback metadata keeps the provisioned client restricted to localhost, and MindRoom rejects a new registration before persistence when the response does not confirm the requested callback.
