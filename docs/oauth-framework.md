...
Agent-facing OAuth tools return the same structured `reset_required` signal and direct the agent to `reset_oauth_connection`, which issues an authenticated, requester-bound browser reset flow instead of an unusable connect link.
