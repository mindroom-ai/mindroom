...
Only agents whose effective worker scope is unscoped or `shared` appear in `/v1/models`.
Agents that use `agents.<name>.private` are not listed there, because `private.per` always uses an isolating scope.
