...
| Matrix voice-message transcription | No | Configure [`voice.stt`](../voice.md) with OpenAI API credentials or an OpenAI-compatible local endpoint |
| Text-to-speech | No | Configure a [TTS tool](../voice.md#text-to-speech-tools) or cascaded call speech service |
| Realtime speech-to-speech calls | No | Configure the [realtime call backend](../voice-calls.md) with a separate OpenAI API credential service |
| Cascaded calls using Codex for reasoning | Yes | Keep the agent on Codex and configure separate STT and TTS services in the [cascaded backend](../voice-calls.md#cascaded-cloud-example) |
