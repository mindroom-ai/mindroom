...
| `MINDROOM_DASHBOARD_ALLOWED_HOSTS` | Comma-separated extra `Host` values an open-access dashboard answers, beyond loopback names, address literals, and the hosts of `MINDROOM_PUBLIC_URL`, `MINDROOM_SCRIPT_GATEWAY_URL`, and `MINDROOM_URL`; `*` accepts any host and gives up the DNS-rebinding defence | _(none)_ |
...
| `MINDROOM_DASHBOARD_CORS_ALLOW_ALL_ORIGINS` | Set to `true` to allow every dashboard API origin while disabling credentialed CORS responses; ignored without `MINDROOM_API_KEY` or platform auth, where every response is administrator data | _(unset)_ |
