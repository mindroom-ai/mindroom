...
Use `tool_approval` to require confirmation before specific tool calls run.
Rules are checked in order, and the first matching rule wins.
Each rule must set exactly one of `action` or `script`.
`action` must be `auto_approve` or `require_approval`.
`script` points to a Python file that exports a `check(tool_name, arguments, agent_name)` function returning a boolean.
Return `True` to require approval for that call.
