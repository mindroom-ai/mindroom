...
Tools like `shell`, `file`, `python`, and `coding` get a default working directory (`base_dir`) inside the agent's workspace, but this is a convenience, not a hard boundary.
