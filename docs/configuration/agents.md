...
When worker-owned execution is active, `memory_file_path` and `context_files` must remain relative.
MindRoom bootstraps those paths into the agent-owned workspace and future prompt/file-memory reads use that worker-owned copy.
