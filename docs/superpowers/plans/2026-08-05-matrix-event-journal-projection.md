...
| 4 streaming progress is transport-only | Not done; belongs with the delivery cutover |
| 5 acknowledged sends are provisional | **Done.** `send_text` and `edit_text` seed on acceptance; an original's echo replaces its provisional row, a revision's echo is recognised by identity and installed unconditionally. A genuine duplicate still changes nothing |
...
| 9 narrow views, one backend | **Done.** Six structural protocols in `event_journal/views.py`; each collaborator takes the slice it calls. Enforcement is the type checker: a hydrator reaching for `enqueue_delivery` fails `ty` before any test runs |
| 10 special facts stay specialized | Pending; sidecar text and approvals are unbuilt |
