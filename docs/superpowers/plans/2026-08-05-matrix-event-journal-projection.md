...
| 4 streaming progress is transport-only | **Not done, and the gap is on the ingress side, not the delivery side.** Nothing seeds a progress edit any more, but every echo of one is admitted and reduced into `visible_messages`, so a streamed answer still costs one projection write per progress edit. The fix is a projection policy that skips a self-authored replacement whose visible content carries a transport-only stream status, while user-authored edits and the completed terminal revision still reduce |
