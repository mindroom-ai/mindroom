...
| 6 hydration is the prompt window | **Not held — overstated here for three rounds.** True of *room* hydration: the unused floor is gone, the window is counted in logical messages rather than pages of events, and the ceiling case is documented as a completion rather than a full window. *Thread* hydration has no ceiling at all. `_fetch_thread` takes the root and then walks the entire recursive relation tree with no limit, accumulating every projected event and installing all of them in one transaction. This product makes that worse than it sounds: a streamed answer is one original plus a long tail of `m.replace` edits, and every edit is a relation, so a thread's relation tree is an order of magnitude larger than its logical message count. The SQL reads are bounded; the first strict read of a large thread is not. Being fixed with the same three ceilings, subject to keeping every admitted message's edit tail intact — a truncation that splits a message from its own edits projects it at a stale revision, which is worse than a shorter window |
...
| 9 narrow views, one backend | **Held in substance; the count in earlier revisions of this row was wrong.** Nine structural protocols in `event_journal/views.py` — `AdmissionView`, `ReplayView`, `DispatchView`, `RelationView`, `ProjectionView`, `ConversationReadView`, `HydrationView`, `OutboxView`, `ApprovalView` — plus `MembershipView` in `event_journal/membership.py`, for ten. Each collaborator takes the slice it calls. Enforcement is the type checker: a hydrator reaching for `enqueue_delivery` fails `ty` before any test runs |
...
What still blocks contract 2 is the durable input snapshot described next, not the absence of a settlement point.
