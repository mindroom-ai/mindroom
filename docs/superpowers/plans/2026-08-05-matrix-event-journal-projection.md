...
**A sixth step, found by building steps 1-5 and reverting them.** The production wiring applies cleanly and the streaming tests then fail — `assert None == '$stream_1'` and similar — because the transform factory reaches `deps.response_hooks._apply_final_response_transform`, and the streaming test doubles do not provide `response_hooks`. The terminal delivery raises inside the closure and returns `None`. So the change carries a test-surface migration: either the doubles gain the hook, or the factory tolerates its absence, and that decision should be made deliberately rather than to make a suite pass. Nothing above is wrong; it is simply not the whole cost.
