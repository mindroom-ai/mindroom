...
The fix is to apply the transform *before* the terminal enqueue, which removes the second edit entirely. The seam is the `terminal_send` closure at `delivery_gateway.py:1317-1347` and its sibling `_durable_terminal_edit`: both receive the already-built `content` and hand it to `_send_content` with `delivery_stage=DeliveryStage.FINAL`. The transform currently runs at `:1734` against `final_body_candidate`, which is available at `:1490` — before the terminal edit is sent — so the value exists early enough; what does not exist yet is a content-level transform seam, because the hook takes `response_text` rather than a content dict. Both terminal paths need it, and the branches around them (cancelled, failed, placeholder, edited-versus-sent) are what make this larger than it looks.
