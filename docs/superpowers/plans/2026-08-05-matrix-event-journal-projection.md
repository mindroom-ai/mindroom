...
The repayment walk is now debt-aware: it keeps walking past a full prompt window until it reaches the anchor, and stops only on both conditions. `LOST` is now reserved for the two cases where it is provable — the server ran out of history, or the walk hit the cost ceiling, where retrying is not merely expensive but non-improving, because the walk restarts from a tip that only moves further away while the anchor stays fixed. Pinned by `test_a_repayment_walks_past_the_prompt_window_to_reach_its_anchor`, which asserts the page beyond the window lands in the projection by value, and by a sibling that pins that the ceiling — not the window — is what stops a genuinely deeper gap.
