...
**One pin is owed and it is not optional.** Removing the pre-enqueue preparation leaves the whole suite green — nothing observes the fix, so it can regress silently. What is needed is a test that an oversized message enqueues a payload already carrying its sidecar: capture what reaches the outbox `deliver(payload=...)` for a send with a `delivery_turn_id`, and assert it is the `m.file` form with the MXC rather than the oversized original. The existing large-message tests all drive `send_message_result` directly and therefore cannot see this. Until that exists, the fix is correct but unprotected.
...
The schema column is the easy half. The decision is the real work, and it should not be made by whoever happens to touch this next: a Matrix transaction ID is scoped to `(device, txn_id)`, so once the device changes the ID carries no dedup at all, and recovery has to pick between refusing to resend — silent loss, which is the failure class this whole plan exists to remove — and resending under a fresh ID, which duplicates a visible answer if the original landed. Neither is acceptable as a default.

Reconciliation is the way out, and the branch already has the piece: `VisibleResponseReconciler` scans Matrix for a turn's visible response. Outbox recovery meeting a device change should ask it rather than guess — acknowledge the row if the answer is already in the room, resend under a fresh ID only if it is not. That is a wiring job between two existing owners, not a new mechanism, and it is why persisting `device_id` alone would be a half-fix that reads like a whole one.
...
| Retry after acceptance is idempotent | A repeated transaction ID returned the *same* event ID, and deduplication proved to be scoped to the sending device -- a different device reusing the ID creates a new event |
