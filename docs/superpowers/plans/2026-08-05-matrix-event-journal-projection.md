...
- A refused FINAL edit left two owners. The failure notice is sent as a plain message rather than through the frozen `m.replace` row, deliberately, but the row stayed attempted and unacknowledged -- so the next recovery pass resent the frozen envelope and the same error text reached the room twice. The fallback is now adopted as the row's outcome, *after* the send: adopting first and crashing would lose the answer outright, because the FINAL enqueue has already handed the journal sources over and nothing would replay the turn.
