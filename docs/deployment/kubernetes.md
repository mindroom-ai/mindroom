...
This section mixes current deployment behavior with intended end-state behavior.
The dedicated-worker provisioning flow is implemented today.
The canonical shared agent-state model described below is the target design, not a claim that every backend path already behaves that way.
...
In the target model, the runner also reads and writes the same canonical agent state roots used by the primary runtime.
...
The target model is that each worker pod runs the sandbox-runner app and is able to access the same canonical agent state roots used by every other runtime for the same agent.
...
For the target model, set `storageAccessMode: ReadWriteMany` so multiple workers can access the same canonical agent state roots concurrently.
