...
The runner still reads and writes the same canonical agent state roots used by the primary runtime.
...
Each worker pod runs the sandbox-runner app and must be able to access the same canonical agent state roots used by every other runtime for the same agent.
...
For multi-node operation, set `storageAccessMode: ReadWriteMany` so multiple workers can access the same canonical agent state roots concurrently.
