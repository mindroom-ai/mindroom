...
If `provisioner.instanceCredentialsEncryptionSecret` is unset, the provisioner falls back to `PROVISIONER_API_KEY`, so rotating the provisioner API key also rotates derived credential encryption keys.
