...
Written objects must carry that tenant's `customer` label, and created or updated names must start with `{kubernetesWorkerNamePrefix}-{customer}-`.
Worker pod templates must carry that tenant's `customer` label, run as the configured worker ServiceAccount with `automountServiceAccountToken: false`, and reference only `mindroom-storage-{customer}`, `mindroom-config-{customer}` and `mindroom-worker-auth-{customer}`.
...
Worker pods may only use the chart's `controlPlaneNodeName` and `kubernetesWorkerRuntimeClassName`, may not set a priority, tolerations or image pull Secrets, and may only be owned by `mindroom-{customer}`.
...
Dedicated-worker names are tenant-scoped, which requires `customer` to be lowercase letters and digits and the normalized `{kubernetesWorkerNamePrefix}-{customer}` prefix to fit in 38 characters; the chart fails to render otherwise.
A hyphen would let one tenant's prefix match another tenant's worker names, and a longer prefix would be truncated by the runtime.
