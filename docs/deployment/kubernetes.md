...
Its Deployment and Service verbs are namespace-wide because worker names are derived from the worker key at runtime, so a per-tenant `ValidatingAdmissionPolicy` confines each tenant's worker manager to its own resources.
...
The policy runs only for `system:serviceaccount:mindroom-instances:mindroom-worker-manager-{customer}` and requires that the object carry that tenant's `customer` label, that created and updated names start with `{kubernetesWorkerNamePrefix}-{customer}-`, that pod templates carry that tenant's `customer` label, run as the configured worker ServiceAccount with `automountServiceAccountToken: false`, and reference only `mindroom-storage-{customer}`, `mindroom-config-{customer}` and `mindroom-worker-auth-{customer}`.
Host namespaces, privileged containers and added capabilities are denied as well.
This needs a Kubernetes 1.30+ API server for `admissionregistration.k8s.io/v1`; on older clusters the release fails to install rather than granting the unconfined Role.
Dedicated-worker names are tenant-scoped for the same reason, so worker Deployments and Services created by an earlier release keep their old names; deleting them stays allowed because the policy authorizes deletes by the tenant's `customer` label.
Tenant-scoped names also require `customer` to be lowercase letters and digits, because a hyphen would let one tenant's prefix match another tenant's worker names; the chart fails to render otherwise.
