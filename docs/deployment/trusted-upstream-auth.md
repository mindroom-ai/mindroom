...
Header-only mode is a compatibility option for deployments where MindRoom is unreachable except through that trusted gateway.
...
If no trusted email header is configured, the trusted user ID header must match the verified JWT claim.
