...
3. For Synapse, manually expose the generated registration file to the Synapse container, set the matching `app_service_config_files` path, and restart it with `./deploy.py restart <name> --only-matrix`; for Tuwunel, complete the printed admin-room steps or run `./bridge.py register-with-matrix <type> --instance <name>`.
