...
### Round 1–2 (Gastown agents, Claude Sonnet 4.6 via LiteLLM)
...
### Round 3 (agent-cli dev + Codex gpt-5.4, sections 1–8)
- `CONF-003`: Removed agent's stale Matrix member not cleaned from rooms after hot-reload — fixed by adding orphan cleanup to `_reconcile_post_update_rooms()`.
- `ROOM-006` (refined): Router in root space still misidentified as orphan — fixed by extending `get_configured_bots_for_room()` to include the router localpart for the managed root space.
- `CMD-009`: Interactive menu/reaction prompt completely broken on edited (non-streaming) responses — fixed by formatting menus and registering reaction handlers before editing the Matrix event.

Round 3 results: **66/72 PASS, 3 FAIL (all fixed), 3 SKIP** across sections 1–8.
