...
- Membership comes from the publisher account's synced room state, so viewer or publisher departure takes effect when that account's sync delivers the change.
- Revocation is read from persistent metadata on every request and therefore takes effect immediately.
