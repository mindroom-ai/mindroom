...
The current repository dependency in `pyproject.toml` is `mindroom-nio[e2e]==1.1`; `uv.lock` resolves version `1.1.0` from the package registry, with no Git source override.
