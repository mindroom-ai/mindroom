...
- **max_tool_calls_per_turn**: Max tool calls one turn may execute (per-agent override of `defaults.max_tool_calls_per_turn`, 500); later calls return a tool error, and the turn ends if the model then asks for tools again
