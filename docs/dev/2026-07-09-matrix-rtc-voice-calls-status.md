...
| mindroom-ai/mindroom | [#1524](https://github.com/mindroom-ai/mindroom/pull/1524) | **open** | Move backend, model, voice, credentials, STT, and TTS settings into per-agent call profiles; the production config must migrate from the former agent list before redeploying |
...
- Mind is the production calls agent; before the next redeploy, rename its config key to `mind` everywhere and set `calls: {enabled: true, agents: {mind: {}}}`.
...
- [ ] Rename Mind's production agent key to `mind` everywhere and migrate its call profile to `calls: {enabled: true, agents: {mind: {}}}`.
- [ ] Redeploy `mindroom-chat` from current main to pick up mindroom-nio 0.27.0, the AgentSession startup fix, and the per-agent call configuration, then live-validate encrypted-room hearing and voice startup.
