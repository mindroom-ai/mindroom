...
MindRoom agents join Element Call (MatrixRTC) voice calls in their rooms and hold real spoken conversations through an OpenAI realtime speech-to-speech model (`gpt-realtime-2.1`), using their authored display name, role, and instructions with a spoken-style addendum, with a transcript saved to the agent workspace and a daily-memory entry per call.
Calls intentionally do not expose agent tools because MatrixRTC audio does not provide a trustworthy Matrix requester identity per speaker.
...
|------|----|-------|----------|
| mindroom-ai/mindroom | [#1452](https://github.com/mindroom-ai/mindroom/pull/1452) | draft, branch `matrix-rtc-voice-calls`; `origin/main` merged in 2026-07-09 (E2EE #1423 etc.), mergeable, nio pin `>=0.27` | `src/mindroom/matrix_rtc/` package: call manager/session, Element Call wire formats, lk-jwt exchange, frame-key rotation, livekit-agents media bridge (`matrix_calls` extra), transcripts + daily memory, PL0 power-level fix, `calls:` config, docs, ~50 tests + live smoke harness |
...
| mindroom-ai/mindroom-cinny | [#96](https://github.com/mindroom-ai/mindroom-cinny/pull/96) | **merged** (deployed to prod earlier from the branch) | Service worker fix: the Workbox app-shell navigation fallback hijacked the Element Call widget iframe (`/public/element-call/index.html`), which was the entire "stuck on Joining" bug; adds `/public/` to the denylist |
...
| Authored voice instructions, transcripts, daily memory | ✅ unit tests | `tests/test_matrix_rtc_transcript.py`, `tests/test_matrix_rtc_call_manager.py` |
...
| gpt-realtime agent actually speaking in a call | ✅ live | A real CallManager agent greeted aloud, understood a spoken question, and wrote a transcript plus daily memory entry |
...
3. **In-call tools could not preserve requester authorization** — MatrixRTC audio does not bind a realtime tool invocation to an authenticated Matrix speaker, which could have run tools as the bot identity.
   Fixed by removing tools from the call surface; calls now use the safe authored voice instructions only.
...
- Calls use the agent's authored display name, role, and instructions instead of the full chat prompt, so the chat prompt preload limit does not affect a cold call session.
...
- [x] Run the full agent-speaking test with the real OpenAI key (bot joins, greets, answers, transcript + daily memory written) — PASS 2026-07-09, see validation matrix.
