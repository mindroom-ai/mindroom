...
Downloads and decrypts media data, then processes it through the agent.
...
**Routing** (when no agent or team is mentioned): Router narrows candidates from room configuration or joined MindRoom entities, filters them by sender permissions, routes directly when one candidate remains, and uses `suggest_agent_for_message()` only when multiple candidates remain.
