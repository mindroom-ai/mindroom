...
2. Sampling the in-flight count and closing the shared `ResponseAdmissionGate` happen atomically, so a new response cannot race the decision to apply. The gate stays closed (but is not held) across config loading and plan application: applying the plan stops bots, and stopping a bot drains its detached responses, so holding the gate there would stall that drain against itself
3. While the gate is closed, a response that tries to start is refused immediately and cancelled rather than blocking, since its entity is about to be stopped
4. If responses never drain, the reload stops deferring after `_CONFIG_RELOAD_DRAIN_FORCE_AFTER_SECONDS` and applies anyway, so a config change cannot be starved forever on a busy install
