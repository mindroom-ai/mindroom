...
- Skill review evidence is the conversation the reviewed agent's model already processed, so its redaction is best-effort, and the check that refuses learned skill files holding a literal credential is a heuristic for common formats; a miss is not a vulnerability, because learned skills live in the agent's own workspace next to the conversation history that already held the value.
