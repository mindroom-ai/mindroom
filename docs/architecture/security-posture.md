...
- A skill review forks the request the reviewed agent's model just processed and resends it unredacted to the same provider, a digest replay, used for another review model or a request that cannot be forked, redacts the conversation on a best-effort basis, and the check that refuses skill files holding a literal credential is a heuristic for common formats; a miss is not a vulnerability, because skills live in the agent's own workspace next to the conversation history that already held the value.
