...
- Skill review evidence is the conversation the reviewed agent's model already processed, so its redaction is best-effort; the enforced check is that learned skill files containing a literal credential are refused when written.
