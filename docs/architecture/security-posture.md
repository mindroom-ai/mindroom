...
- Automatic skill learning trusts the `metadata.mindroom.learned` frontmatter marker and `skills/.usage.json` inside the workspace, which worker code can change; that only lets it make the learner edit or archive skills it could already change itself.
