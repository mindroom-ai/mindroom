...
- A skill review forks the request the reviewed agent's model just processed and resends it unredacted to the same provider, a digest replay for another model redacts the conversation on a best-effort basis, and the check that refuses skill files holding a literal credential is a heuristic for common formats; a miss is not a vulnerability, because skills live in the agent's own workspace next to the conversation history that already held the value.
...
- `skill_manage`, which learning agents use in chat and the review writes with, runs in the primary process and writes only below the agent's workspace `skills/` directory through the same no-follow descriptors, regardless of `file_access`, like MindRoom-owned output.
