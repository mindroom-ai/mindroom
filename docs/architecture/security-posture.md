...
| Code-execution tools | `shell`, `python`, `docker`, `script`, `claude_agent` | Always `unrestricted`; authored config may only state `file_access: unrestricted` |
| Other tools | Everything else | Take no local file paths |

MindRoom logs a warning when an agent routes code tools to a worker but sets `file_access: unrestricted`, because its primary-process path tools can then read runtime secrets.
The model sees the effective file access in its tool execution environment description.
