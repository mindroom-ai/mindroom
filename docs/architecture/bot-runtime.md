...
Reaction and numeric-answer admission use a content-free order ledger, created only for Matrix targets that exposed a prompt, to snapshot the exact prompt revision visible at the source event's Matrix order; ordinary streaming edits retain no history.
