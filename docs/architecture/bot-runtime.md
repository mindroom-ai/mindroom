...
A joined sync normally removes the invite cache entry, so cache absence after a successful join does not revoke acceptance, while a still-present different invite makes the attempt fail closed.
