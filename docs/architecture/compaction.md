...
| --- | --- |
| `history/agno_message_builder_patch.py` | Preserves roleful Team inputs and strips historical inline media in Agent/Team message builders; installed explicitly by `agents._initialize_agent_instance` and `teams._create_team_instance`. |
| `agno_session_persistence_patch.py` | Guards and adapts Agno's asynchronous persistence against the owned session store; installed by `agent_storage._create_sqlite_state_storage` before filesystem work. |
| `history/agno_prompt_compat.py` | Calls the private Agno Team tool preparer and temporarily supplies tool instructions, restoring the original list even on failure. |
...
| `history/summary_provider_compat.py` | Normalizes effective Claude request overrides, preserves shorter HTTP limits and injected transports, disables nested retries, and detects truncated output. |
...
- Claude summaries stopped by an output or context limit cannot be committed as complete summaries.
...
- The outer retry policy disables Agno-level retries; Claude summary adapters also disable SDK retries.
