...
| --- | --- | --- |
| `custom_tools/agno_compat_moviepy.py` | Caption style forwarding, measured canvas sizing, bottom placement, and non-overlapping word wrapping; SDK parsing and temporary-file helpers remain shared. | `tools/moviepy_video_tools.py` selects the adapter lazily; upstream output publication, rendering settings, and cleanup behavior remain intact. |
