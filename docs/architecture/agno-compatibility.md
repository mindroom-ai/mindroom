...
| --- | --- | --- |
| ModelsLab polls a local artifact UUID and reports unsuccessful waits as success. | Poll the provider job ID and preserve completion, timeout, and provider error outcomes. | `tools/agno_compat_modelslabs.py`; `tests/test_modelslabs_tool.py`. |
...
| --- | --- | --- |
| `tools/agno_compat_modelslabs.py` | Generation orchestration retains provider job identity and reports the completion outcome while reusing SDK payload and media helpers. | `tools/modelslabs.py` retains tool registration, optional imports, and authored file-type normalization; nonwaiting generation stays upstream. |
