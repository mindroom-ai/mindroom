...
| `tool_jobs/agno_compat_execution.py` | Approved FunctionCall driver binding, provider-schema projection, exact-call native requirements, and saved wait-metadata binding before agent/team tool preparation. | `tool_jobs/agno_execution.py`, `wait_timeout.py`, and `runtime.py` own admission, accepted wait semantics, execution, result claims, and cancellation. |
