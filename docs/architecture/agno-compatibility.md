...
| `agno_compat_model_hooks.py` | Model message projection, post-tool formatting/media, asynchronous post-batch checkpoint chaining, and resumed-response callbacks, the per-loop model request gate, permanent and scoped invocation/retry bindings, answer-cache bypass, and provider client factories. | Approval receipts, queued notices and mid-turn judgments, the model-call cap and its warning, the skill review's input budget, request logging, media fallback, provider stream retries, prompt caching, and participation decisions remain in their owning modules. |
