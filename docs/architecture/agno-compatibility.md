...
| --- | --- | --- |
| MoviePy caption rendering assumes an Arial font file, ignores advertised styles, and keeps fixed caption geometry with an overlapping wrap cursor. | Use a portable font default, forward styles, size and position captions from rendered clip bounds, and advance wrapped words; caption tracking remains unverified. | `custom_tools/agno_compat_moviepy.py`; regressions in `tests/test_moviepy_video_tools.py` and `tests/test_moviepy_caption_layout.py`. |
...
| --- | --- | --- |
| `custom_tools/agno_compat_moviepy.py` | Portable caption font defaults, style forwarding, measured canvas sizing, bottom placement, and non-overlapping word wrapping; SDK parsing and temporary-file helpers remain shared. | `tools/moviepy_video_tools.py` selects the adapter lazily; upstream output publication, rendering settings, and cleanup behavior remain intact. |
