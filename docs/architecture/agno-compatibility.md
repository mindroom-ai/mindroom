...
| Tool preparation does not expose the per-run Function copy the provider calls. | Expose the bound per-run Function from public preparation; no issue or PR identified. | `agno_compat_prepared_tools.py`; BashPresentationFunction reports its prepared copy so the CLI checkpoint can match provider calls exactly. |
| ModelsLab polls a local artifact UUID and discards completed URLs and wait outcomes. | Poll the provider job ID, accept numeric ETAs, retain completed outputs, and distinguish retryable status failures from terminal rejection. | `tools/agno_compat_modelslabs.py`; `tests/test_modelslabs_tool.py`. |
| MoviePy caption rendering assumes an Arial font file, ignores advertised styles, clips glyph bounds, overlaps wrapped words, and leaves temporary audio after encoding failures. | Use a portable font default, forward styles, preserve complete glyph bounds, advance wrapped words, and clean owned audio files on every exit; upstream tracking remains unverified. | `custom_tools/agno_compat_moviepy.py`; regressions in `tests/test_moviepy_video_tools.py`, `tests/test_moviepy_caption_layout.py`, and `tests/test_moviepy_caption_output.py`. |
...
| LocalSkills opens SKILL.md and support files by pathname and follows links, and the Skills tool entrypoints are private methods with no load callback. | Tracking gap: accept a caller-supplied no-follow reader for skill files, support-file discovery, and support-file reads, plus a skill-load callback; no matching issue or PR identified, and [PR #9194](https://github.com/agno-agi/agno/pull/9194) only adds a database loader. | `tool_system/workspace_skills.py` and the `MindroomSkills` entrypoint overrides in `tool_system/skills.py`; link, FIFO, swapped-reference, and usage coverage in `tests/test_skills.py`. |
| FileTools content search relativizes hits and exclusions against `base_dir`, so it cannot search other directories. | Tracking gap: accept an absolute search directory outside `base_dir` and report absolute hit paths; no matching issue or PR identified. | `tools/file.py`; unrestricted outside-directory coverage in `tests/test_coding_tools.py`. |
...
| Terminal error and cancellation cleanup keep stale checkpoint or continuation messages. | Refresh the saved message list from current in-flight messages while retaining cancellation cleanup; no matching upstream tracking identified. | `agno_compat_run_messages.py`; Agent/Team snapshots and resumed-run export coverage in `tests/test_agno_compat_run_messages.py`. |
| Interrupted model streams accumulate usage before retaining the assistant message. | Preserve the metered message before terminal persistence; [issue #9489](https://github.com/agno-agi/agno/issues/9489) identifies the same lost-message boundary, but does not separately track request-detail preservation. | `agno_compat_run_messages.py`; real SDK stream failures and task cancellation in `tests/test_openai_responses_stream.py`. |
| Assistant request messages omit their model, although a resumed run can contain usage from multiple models. | Tracking gap: persist the model ID and provider with each request's counters; no matching upstream issue or PR identified. | `agno_compat_run_messages.py` records attribution at message population and stream entry, including metered failures; `tests/test_request_usage.py` and `tests/test_openai_responses_stream.py` cover mixed-model request exports and daily dates after conversation cleanup. |
| Stopping an async run between chunks abandons its suspended model stream until garbage collection, after the run is persisted without the usage that stream already received. | Close in-flight model streams innermost first before terminal cancellation or error persistence; [issue #9489](https://github.com/agno-agi/agno/issues/9489) covers related abandoned-generation bookkeeping, not usage settlement. | `agno_compat_run_messages.py` settles the abandoned request once from received stream data; consumer closure and failure, cancellation-request, and retried-attempt regressions in `tests/test_openai_responses_stream.py`. |
...
| A response loop's final request is not observable: `RunOutput.messages` drops messages kept out of agent memory, and the tool definitions stay inside `aresponse`, so the skill review cannot fork the request to reuse its prompt cache. | Tracking gap: report each response loop's final messages, tools, tool choice, and response format to a scoped callback; no matching issue or PR identified. | `agno_compat_model_hooks.py`; `skill_learning/capture.py` owns the capture and `skill_learning/reviewer.py` the fork. |
| Response loops call the model for as long as it requests tools: `tool_call_limit` only refuses counted calls, and calls to unknown tools or with unparseable arguments bypass it; callers also have no public hook to end a run before a request, which the skill review's input budget needs. | End every loop after a bounded number of model requests through the normal completion path and offer a public pre-request hook that can end the run; refused `tool_call_limit` batches are tracked in [issue #8304](https://github.com/agno-agi/agno/issues/8304) and [issue #10041](https://github.com/agno-agi/agno/issues/10041), whose open PRs [#10042](https://github.com/agno-agi/agno/pull/10042) and [#8324](https://github.com/agno-agi/agno/pull/8324) stop only after limit refusals, no upstream issue was identified for unknown-tool or unparseable-argument loops, and the pre-request hook is a tracking gap with no issue or PR identified. | `agno_compat_model_hooks.py`; real Agent and Team loops in `tests/test_tool_call_budget.py`, and the review budget in `tests/test_skill_learning.py`. |
...
| TrafilaturaTools extraction and crawling download through fixed module-level fetchers, and Trafilatura's spider has no fetch hook. | Tracking gap: accept a caller-supplied fetcher for extraction, batch extraction, and every crawler download; no matching issue or PR identified. | `tools/agno_compat_trafilatura.py`; loopback, metadata, redirect, DNS-rebind, and crawl coverage in `tests/test_trafilatura_tool.py`. |
...
| ReplicateTools checks its API key but calls an independent SDK default client. | Bind SDK requests to the toolkit key and expose client lifetime ownership while preserving media results. | `custom_tools/agno_compat_replicate.py`. |
...
| --- | --- | --- |
| `tools/agno_compat_modelslabs.py` | Generation orchestration polls provider job identities, accepts numeric ETAs, retains completed outputs, and distinguishes retryable status failures from terminal rejection while reusing SDK payload and media helpers. | `tools/modelslabs.py` retains tool registration, optional imports, and authored file-type normalization; nonwaiting generation stays upstream. |
| `custom_tools/agno_compat_moviepy.py` | Portable caption fonts, style forwarding, complete glyph bounds, measured canvas sizing, bottom placement, non-overlapping wrapping, and owned temporary audio cleanup; SDK parsing and temporary-file helpers remain shared. | `tools/moviepy_video_tools.py` selects the adapter lazily; rendering settings and atomic output publication are preserved. |
...
| `agno_compat_run_messages.py` | Version-guarded Agent/Team terminal snapshots, retention and model attribution of metered request messages, and one-time settlement of abandoned async model streams. | `agent_storage.py` installs the patch; provider adapters own reported counters, and `usage_storage.py` retains content-free projection and retry exclusions. Aggregate accounting remains with Agno and the session-metrics repair. |
...
| `agno_compat_model_hooks.py` | Model message projection, post-tool formatting/media, asynchronous post-batch checkpoint chaining, and resumed-response callbacks, the per-loop model request gate, permanent and scoped invocation/retry bindings, the scoped response-loop observer, answer-cache bypass, and provider client factories. | Approval receipts, queued notices and mid-turn judgments, the model-call cap and its warning, the skill review's input budget and its fork of the final request, request logging, media fallback, provider stream retries, prompt caching, and participation decisions remain in their owning modules. |
| `agno_compat_cli_checkpoint.py` | Task-scoped real provider batch snapshots, including resolved fallback instances, keyed by exact FunctionCall identity. The response lifetime resolves lazy prepared bindings per pull; completed sibling Bash media stays in native result messages. | `agent_cli/turn.py` owns admission and calls the existing propagating `agent_storage` boundary before binding or effects; `cli_approval_recovery.py` persists interrupted-replay sessions through the same `save_cli_session` boundary. |
| `agno_compat_prepared_tools.py` | Private Agent/Team tool preparation, temporary instruction restoration, call-local result-stream/thread lifetime ownership, per-execution stop signaling, and reporting the Bash facade's actual per-run Function copy. | `history/prompt_tokens.py` owns estimation/cache policy; `matrix_rtc/call_tools.py` owns voice policy; `tool_system/agent_tool_calls.py` owns qualified live-turn bindings and serialized execution. |
| `history/agno_compat_message_builder.py` | Roleful Team input and history-media message-builder bindings. | Agent/Team construction installs the patch; history owners keep the newest four marked viewed images within a 10 MiB aggregate limit, disclose omissions without changing stored artifacts, and charge provider visual estimates or a conservative fallback in replay planning. |
...
| `tools/agno_compat_trafilatura.py` | Replaces the Agno and Trafilatura spider download bindings with fetchers that request uncompressed bodies and validate every URL, redirect hop, and dialed address within Trafilatura's timeout, redirect, and size limits. | `tools/trafilatura.py` installs the guard before returning the toolkit; upstream extraction, crawling, and result formats are unchanged. |
...
| `custom_tools/agno_compat_replicate.py` | Copied media-call boundary with a credential-bound Replicate client, inherited output parsing, and private SDK transport cleanup. | `tool_system/metadata.py` retains scoped credential resolution; `tools/replicate.py` retains lazy registration, and the adapter closes each request client after consuming outputs. |
...
| `tool_system/workspace_skills.py` | Builds Agno `Skill` fields for workspace roots from descriptor-bound reads instead of `LocalSkills`. | Allowlists, OpenClaw eligibility, and workspace precedence stay in `tool_system/skills.py`. |
| `tool_system/skills.py` | `MindroomSkills` overrides the private skill-tool entrypoints for workspace skills: confined support-file reads, usage records, and blocked script execution. | Configured skills keep Agno's entrypoints unchanged; the learner's archival policy lives in `skill_learning/library.py`. |
| `tools/file.py` | Unrestricted outside-directory content search through a shallow copy rooted at the searched directory, with hits rewritten to absolute paths. | Workspace-mode refusal, blocked-path messages, and Agno's inside-workspace search remain unchanged. |
