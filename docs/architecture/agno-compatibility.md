...
| Team run outputs omit tool-mutated session state without an explicit input seed. | Persist the initialized run context in team outputs without requiring callers to seed it. | `teams.py` seeds state only inside managed execution scopes; `tests/test_tool_job_turn_integration.py` covers stored receipts and ordinary SDK output. |
...
| `agno_compat_model_hooks.py` | Model message projection, post-tool formatting/media, asynchronous post-batch checkpoint chaining, and resumed-response callbacks, permanent and scoped invocation/retry bindings, answer-cache bypass, and provider client factories. | Approval receipts, queued notices and mid-turn judgments, request logging, media fallback, provider stream retries, prompt caching, and participation decisions remain in their owning modules. |
...
| `tool_jobs/agno_compat_execution.py` | Approved FunctionCall driver binding, provider-schema projection, exact-call native requirements, saved wait-metadata binding before agent/team tool preparation, and per-call completion ownership for nested SDK dispatch inside an accepted job. | `tool_jobs/agno_execution.py`, `wait_timeout.py`, and `runtime.py` own admission, accepted wait semantics, execution, result claims, and cancellation; internal SDK model calls keep their existing owner and inherited authority. |
