...
`memory_file_path` seeds the OpenClaw workspace into the agent's canonical workspace root, so the canonical `MEMORY.md` is loaded automatically by the file backend as the entrypoint.
Every runtime for that agent reads and writes that same canonical copy, so there is no need to list `MEMORY.md` in `context_files`.
