...
- **Conversation history** is stored in Agno sessions, but MindRoom decides what replay summary and raw history messages are injected into each run. Control replay depth with `num_history_runs` or `num_history_messages`, and optional required compaction with `compaction` (see [Agents](configuration/agents.md)).
