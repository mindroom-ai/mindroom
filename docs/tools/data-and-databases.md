...
Constructors accept only common parsing keywords, so `engine`, `engine_kwargs`, `compression`, `storage_options`, and `chunksize` are rejected.
...
- Every conversation that uses the same agent workspace shares its dataframe names.
