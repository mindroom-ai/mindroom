...
Ephemeral participant `tools` must be empty until explicit workflow tool grants exist.
...
Permissions currently support runtime and model caps.
`permissions.tools` must be empty, and `permissions.data` must keep `matrix_history`, `attachments`, and `knowledge_bases` disabled until approval-backed data grants exist.
        ...
        "workflow": [
            {
                ...
                "type": "agent_step",
                "participant": "writer",
                "prompt": "Write a concise cited report about {input.topic}.",
            },
        ],
