...
Hourly cleanup replaces older results with execution receipts only after the originating turn has finished and the conversation has no pending approval continuation.
...
An expired result reports its expiry, and its retained receipt prevents the original tool call from executing again.
Receipts retain invocation identity and authorization metadata, so their storage still grows with execution history.
