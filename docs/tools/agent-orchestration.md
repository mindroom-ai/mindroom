...
Use these tools when you need multi-agent coordination, reusable workflow runs, runtime config changes, config-only presets, persistent Claude Agent SDK sessions, or retained usage statistics.
...
All nine entries on this page are MindRoom-native orchestration features rather than third-party OAuth integrations.
...
`usage_stats` returns aggregate retained-run usage for the caller without modifying session storage.
...
The result covers retained Agno runs only, excludes the in-flight tool call, and can be incomplete after compaction.
...
Authorized admin queries attribute each team coordinator run to its configured team ID and each nested member run to that member's configured agent or nested-team ID.
...
A successful response also includes `scope`, `window`, `as_of`, token `totals`, `cost`, `turn_count`, `run_count`, `session_count`, observed timestamps, status counts, a `breakdown`, truncation fields, and `coverage`.
`turn_count` counts accepted top-level agent or team turns, while `run_count` counts retained runs with usable metrics and can also include nested team-member runs.
Token totals separately report input, output, cache-read, cache-write, reasoning, and audio dimensions.
Cost is a decimal-string subtotal accompanied by counts of runs with and without retained cost data; it is not a provider invoice.
Model breakdown rows identify the model type, provider, and model ID separately.

Breakdown rows are sorted by total tokens and capped at 200.
`breakdown_truncated` and `breakdown_omitted` state whether rows were left out, while the top-level totals still cover every included run.
Coverage can be `complete_retained`, `partial`, or `unknown`.
`complete_retained` means no gap was detected in the retained data, not that provider-billed history is complete.
Unreadable sources, malformed records, missing attribution, timestamps, or stable run IDs, and detected compaction gaps produce partial coverage and corresponding bounded counters.
Discovery, per-source rows, decoded run nodes, metric fan-out, selected identifiers, and request wall-clock work all have fixed bounds.
Per-source bound exhaustion produces partial coverage when safe records remain, while request-wide exhaustion returns partial admin results and a source-unavailable error for self queries.
Shared-agent self queries can report unknown completeness because cumulative session totals cannot safely be compared across requesters.

Errors use the same envelope with a stable code.
Common codes are `validation_error`, `authorization_error`, `context_unavailable`, and `source_unavailable`.
