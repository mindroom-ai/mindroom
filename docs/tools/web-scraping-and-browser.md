...
It manages named browser profiles, with `openclaw` as the default profile name.
...
Only `target="host"` is supported on this branch, so OpenClaw compatibility fields for sandbox or node targeting currently return an error.
...
- Use the dedicated [OpenClaw Workspace Import](../openclaw.md) guide for the broader OpenClaw compatibility story and preset behavior.
