...
To let the user watch this worker browser, use `chat_ui.open_panel(panel='computer')` with the opt-in [Chat UI toolkit](chat-ui.md).
...
Host `upload` reads only files inside that artifact directory, or the worker workspace on a routed worker, so `output_dir` must not contain credentials, encryption keys, received attachments, or other runtime state.
