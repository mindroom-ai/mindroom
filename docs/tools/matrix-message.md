...
Check its `thread_mode` to choose where to read replies and send follow-ups.
For a target with `thread_mode="thread"`:
...
For a target with `thread_mode="room"`, use the room timeline for both the initial message and follow-ups:
...
matrix_message(action="read", thread_id="room")
matrix_message(
    action="send",
    thread_id="room",
    message="@ops:example.org Include the first three commands to run.",
