...
`generate_video()` uses the configured `video_generation_model`, which defaults to `veo-3.1-generate-preview`, polls until the long-running operation completes, and returns attached video artifacts.
