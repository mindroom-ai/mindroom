...
`spotify` is the only tool on this page with dedicated integration routes in [src/mindroom/api/integrations.py](/srv/mindroom-worktrees/issue-075-docs-split/src/mindroom/api/integrations.py).
...
The tool itself consumes an `access_token`, but MindRoom also provides a dedicated dashboard OAuth flow in [src/mindroom/api/integrations.py](/srv/mindroom-worktrees/issue-075-docs-split/src/mindroom/api/integrations.py) via `/api/integrations/spotify/connect`, `/spotify/status`, `/spotify/callback`, and `/spotify/disconnect`.
