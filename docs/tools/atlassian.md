...
Site URLs must use HTTPS, and cloud IDs must be UUIDs, so a misconfigured value fails when the tool is built.
...
| `client_config_service` | `atlassian_oauth_client` | Credential service holding the app client ID and secret. |
...
Register each connection's callback URL on its Atlassian app before users connect.
By default, every connection uses the same app through `atlassian_oauth_client`.
If that app cannot hold another callback URL, or the site belongs to a different organization with its own app, create a separate app and set `client_config_service` to a service such as `partner_atlassian_oauth_client` that holds its client ID and secret.
...
- Requests go only to `https://api.atlassian.com/ex/{product}/{cloud_id}/...` for the pinned site, and cloud IDs from Atlassian are accepted only in UUID form.
...
- A gateway path containing dot segments, encoded or double-encoded slashes, backslashes, or `;` parameters is treated as foreign and rejected.
...
- Attachment downloads are limited to the inline worker transfer size, 16 MiB by default, controlled by `MINDROOM_ATTACHMENT_INLINE_SAVE_MAX_BYTES`.
