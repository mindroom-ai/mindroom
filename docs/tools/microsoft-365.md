...
Excel parses written text as if typed, so text such as `0042`, `12%`, `1/2`, or `TRUE` would silently become a number, date, or boolean.
...
An existing file is never replaced: the tool picks the first free name among `Name.xlsx`, `Name (2).xlsx`, and so on, and uploads with Graph's `rename` conflict behavior.
