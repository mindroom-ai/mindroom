...
| FLOW-001 / rank 12: Scheduled task restore parsing | Draft PR opened; accepted by line gate | [#908](https://github.com/mindroom-ai/mindroom/pull/908) | `refactor/scheduled-task-restore-parsing` | Production net `-19` lines | `uv run pytest tests/test_scheduling.py tests/test_scheduled_task_restoration.py tests/api/test_schedules_api.py -n 0 --no-cov -q`; `uv run pre-commit run --files ...` |
