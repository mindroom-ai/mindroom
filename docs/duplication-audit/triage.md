...
| API-001 / rank 10: API snapshot publishing | Draft PR opened; accepted by line gate | [#900](https://github.com/mindroom-ai/mindroom/pull/900) | `refactor/api-snapshot-publishing` | Production net `-59` lines | `uv run pytest tests/api/test_api.py -n 0 --no-cov -q`; `uv run pre-commit run --files ...` |
| STORE-001 / rank 11: Knowledge index metadata codec | Draft PR opened; accepted at threshold after revision | [#901](https://github.com/mindroom-ai/mindroom/pull/901) | `refactor/knowledge-index-metadata` | Production net `+20` lines | `uv run pytest tests/test_knowledge_index_metadata.py tests/test_knowledge_manager.py::test_index_metadata_without_source_signature_is_unavailable_and_schedules_refresh tests/test_knowledge_manager.py::test_metadata_state_alone_serves_published_index tests/test_knowledge_manager.py::test_stale_metadata_without_collection_returns_unavailable_index tests/test_knowledge_manager.py::test_published_metadata_write_uses_unique_temp_and_cleans_failed_replace tests/api/test_knowledge_api.py::test_knowledge_status_reads_index_metadata_without_initializing -q -n 0 --no-cov`; `uv run pre-commit run --files ...` |
...
| API-003 / rank 13: Credentials worker-target resolver | Draft PR opened; accepted by line gate | [#904](https://github.com/mindroom-ai/mindroom/pull/904) | `refactor/credentials-worker-target` | Production net `-13` lines | `uv run pytest tests/api/test_credentials_api.py tests/api/test_oauth_api.py -q -n 0 --no-cov`; `uv run pre-commit run --files ...` |
| CRED-002 / rank 14: Google OAuth provider factory skeleton | Draft PR opened; accepted by line gate | [#903](https://github.com/mindroom-ai/mindroom/pull/903) | `refactor/google-oauth-provider-factory` | Production net `-11` lines | `uv run pytest tests/test_google_oauth_providers.py tests/test_google_calendar_oauth_tool.py tests/test_google_drive_oauth_tool.py tests/test_google_sheets_oauth_tool.py tests/api/test_oauth_api.py -q -n 0 --no-cov -p no:tach`; `uv run pre-commit run --files ...` |
...
| WORK-002 / rank 16: Worker lifecycle pure helpers | In progress with Zeno the 5th (`019dfb50-729e-7c70-8ab8-3bf8d60b731d`) | Pending | `refactor/worker-lifecycle-helpers` | Pending | Pending |
...
| MAT-001 / rank 18: Matrix cache backend policy | In progress with Galileo the 5th (`019dfb53-209e-7be1-bb8a-85e29f979318`) | Pending | `refactor/matrix-cache-policy` | Pending | Pending |
