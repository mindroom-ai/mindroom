...
export async function POST(request: Request) {
  try {
    ...
    if (!stopResponse.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to stop instance', details: error },
        { status: 500 }
      )
    }
    ...
    // Only update instance status if stop was successful
    ...
    if (updateError) {
      ...
      // Still return success since the stop worked
    }

    return NextResponse.json({
      success: stopData.success,
