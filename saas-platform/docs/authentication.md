...
2. **Supabase auth** is enabled by the instance's Supabase URL and anon key.
   ...
   Every user of the Supabase project holds a valid token, so `ACCOUNT_ID` is required: without it the runtime logs an error at startup and answers every dashboard request with HTTP 503.
   Hosted provisioning sets that account ID for the customer instance and rejects provisioning requests without one.
3. **Standalone auth** applies without Supabase or trusted upstream auth; `ACCOUNT_ID` alone does not leave standalone mode.
...
- Instance chart: `supabaseUrl`, `supabaseAnonKey`, and `accountId` configure hosted Supabase authentication and its account check; the chart fails to render when only one Supabase value is set or both are set without `accountId`.
...
- API `503` reporting that `ACCOUNT_ID` is not set: set it to the instance owner's Supabase user ID (chart value `accountId`).
