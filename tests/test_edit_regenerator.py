...
@pytest.mark.asyncio
async def test_newer_edit_committed_first_suppresses_stale_older_edit(tmp_path: Path) -> None:
    """A stale edit reaching the lock after a newer one committed is suppressed.

    Edit callbacks run as independent tasks and reorder before reaching the
    response lifecycle lock, so the last one to acquire the lock edits the visible
    response last -- even when it carries the older Matrix revision (the root:3
    live-gate race). The under-lock revision commit must reject that stale edit
    rather than let it clobber the newest body.
    """
    record = _turn_record()
    harness = _harness(tmp_path, turn_record=record)
    older, older_info = _edit_event(event_id="$edit-old:example.org", server_timestamp=1_000_010, new_body="old body")
    newer, newer_info = _edit_event(event_id="$edit-new:example.org", server_timestamp=1_000_020, new_body="new body")

    await _handle_edit(harness, older, older_info)
    await _handle_edit(harness, newer, newer_info)

    commit_older = harness.generate_response.await_args_list[0].args[0].commit_current_revision_under_lock
    commit_newer = harness.generate_response.await_args_list[1].args[0].commit_current_revision_under_lock

    # The newer edit reaches the lock first and commits; the older edit then
    # reaches the lock and must be rejected as superseded.
    assert commit_newer() is True
    assert commit_older() is False


@pytest.mark.asyncio
async def test_older_edit_committed_first_then_newer_still_wins(tmp_path: Path) -> None:
    """Both edits proceed when the older reaches the lock first, newest last."""
    record = _turn_record()
    harness = _harness(tmp_path, turn_record=record)
    older, older_info = _edit_event(event_id="$edit-old:example.org", server_timestamp=1_000_010, new_body="old body")
    newer, newer_info = _edit_event(event_id="$edit-new:example.org", server_timestamp=1_000_020, new_body="new body")
    ...
    # Older acquires the lock first and runs, newer acquires it after and runs
    # last -- so the final visible body is the newest revision.
    assert commit_older() is True
    assert commit_newer() is True


@pytest.mark.asyncio
async def test_duplicate_edit_delivery_is_suppressed_after_commit(tmp_path: Path) -> None:
    """A redelivered identical edit callback does not regenerate a second time."""
    record = _turn_record()
    harness = _harness(tmp_path, turn_record=record)
    event, event_info = _edit_event()
    ...
    first = harness.generate_response.await_args_list[0].args[0].commit_current_revision_under_lock
    second = harness.generate_response.await_args_list[1].args[0].commit_current_revision_under_lock

    assert first() is True
    # Identical revision (same timestamp and event id) is not strictly greater,
    # so the duplicate is rejected rather than re-editing the response.
