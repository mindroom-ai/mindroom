    ...
    turn_store.record_turn(
        TurnRecord.create(
            ...
            source_event_metadata=(
                {event_id: SourceEventMetadata(sender=requester_id) for event_id in source_event_ids}
                if source_event_metadata is None and len(source_event_ids) > 1 and requester_id is not None
                else source_event_metadata
            ),
...
def _source_metadata(*source_event_ids: str) -> dict[str, SourceEventMetadata]:
    return {source_event_id: SourceEventMetadata(sender="@user:example.com") for source_event_id in source_event_ids}
