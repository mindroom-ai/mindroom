...
class TestMultiAgentOrchestrator:
    ...
    async def test_orchestrator_invite_agents_to_room(
        ...
        # Mock ensure_user_account to set proper user_ids
        for name, bot in orchestrator.agent_bots.items():
            bot.ensure_user_account = AsyncMock()
            bot.agent_user.user_id = f"@mindroom_{name}:localhost"

        mock_inviter_client = AsyncMock()
        await orchestrator.invite_agents_to_room("!room:localhost", mock_inviter_client)

        # Verify invites were sent for all agents (3 total: calculator, general, router)
