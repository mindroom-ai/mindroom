...
class TestMultiAgentOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_start_generates_avatars_before_starting_bots(self, tmp_path: Path) -> None:
        """Avatar generation should happen before any bot startup when enabled."""
        events: list[str] = []
        orchestrator = MultiAgentOrchestrator(storage_path=tmp_path, generate_avatars=True)
        orchestrator.config = MagicMock()

        async def _router_start() -> bool:
            events.append("router_start")
            return True

        async def _general_start() -> bool:
            events.append("general_start")
            return True

        router_bot = MagicMock()
        router_bot.agent_name = "router"
        router_bot.try_start = AsyncMock(side_effect=_router_start)

        general_bot = MagicMock()
        general_bot.agent_name = "general"
        general_bot.try_start = AsyncMock(side_effect=_general_start)
        ...
        async def _setup(_bots: list[object]) -> None:
            events.append("setup")

        async def _sync_room_avatars() -> None:
            events.append("sync_rooms")

        with (
            patch("mindroom.orchestrator.wait_for_matrix_homeserver", new=AsyncMock()),
            patch.object(orchestrator, "_generate_avatars_if_enabled", side_effect=_generate),
            patch.object(orchestrator, "_setup_rooms_and_memberships", side_effect=_setup),
            patch.object(orchestrator, "_sync_room_avatars_if_enabled", side_effect=_sync_room_avatars),
            patch.object(orchestrator, "_schedule_knowledge_refresh", new=AsyncMock()),
            patch.object(orchestrator, "_sync_memory_auto_flush_worker", new=AsyncMock()),
            patch("mindroom.orchestrator.sync_forever_with_restart", new=AsyncMock()),
        ):
        ...
        assert events[:4] == ["generate", "router_start", "general_start", "setup"]
