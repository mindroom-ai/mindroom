...
class TestMultiAgentOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_update_config_replays_unsynced_tool_approval_resolution_after_restarted_bot_first_sync(
        self,
        tmp_path: Path,
    ) -> None:
        """Hot reload should drain unsynced approval edits when the restarted bot reaches first sync."""
        runtime_paths = TestAgentBot._runtime_paths(tmp_path)
        ...
        new_config = _runtime_bound_config(
            Config(
                agents={
                    "general": {
                        "display_name": "GeneralAgent",
                        "role": "Updated general assistant",
                        "model": "default",
                        "rooms": ["lobby"],
                    },
                },
        ...
        restarted_bot.try_start = AsyncMock(side_effect=_start_bot)

        async def _emit_bot_ready(_response: object) -> None:
            await orchestrator._handle_bot_ready(restarted_bot)

        restarted_bot._on_sync_response = AsyncMock(side_effect=_emit_bot_ready)

        plan = ConfigUpdatePlan(
            new_config=new_config,
            changed_mcp_servers=set(),
            all_new_entities={"general"},
            entities_to_restart={"general"},
            new_entities=set(),
        ...
        try:
            with (
                ...
                patch("mindroom.orchestrator.create_bot_for_entity", return_value=restarted_bot),
    ...
    @pytest.mark.asyncio
    async def test_sender_room_reconcile_finalizes_approvals_before_leaving_room_without_client_room_cache(
        self,
        tmp_path: Path,
    ) -> None:
        """Forced finalization should still edit the approval card before first sync hydrates client.rooms."""
        runtime_paths = TestAgentBot._runtime_paths(tmp_path)
        orchestrator = MultiAgentOrchestrator(runtime_paths=runtime_paths)
        orchestrator._capture_runtime_loop()
        ...
        sender = AsyncMock(
            return_value=SentApprovalEvent(
                event_id="$approval",
        ...
        assert tuple(code_bot.client.rooms) == ()
        ...
        try:
            with (
                patch.object(
                    store,
                    "acknowledge_unsynced_request",
                    wraps=store.acknowledge_unsynced_request,
                ) as mock_ack,
                patch("mindroom.bot_room_lifecycle.get_joined_rooms", new=AsyncMock(return_value=["!room:localhost"])),
                patch("mindroom.bot_room_lifecycle.leave_non_dm_rooms", new=leave_non_dm_rooms),
            ):
