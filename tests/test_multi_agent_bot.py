...
class TestMultiAgentOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_router_finalizes_pending_approvals_before_leaving_room(self, tmp_path: Path) -> None:
        """Router-owned approval cards should be resolved before the router leaves their room."""
        runtime_paths = TestAgentBot._runtime_paths(tmp_path)
        orchestrator = MultiAgentOrchestrator(runtime_paths=runtime_paths)
        orchestrator._capture_runtime_loop()
        ...
        try:
            with (
                ...
                patch("mindroom.bot_room_lifecycle.leave_non_dm_rooms", new=AsyncMock(side_effect=_leave_rooms)),
            ):
            ...
            router_bot.client.room_send.assert_awaited()
