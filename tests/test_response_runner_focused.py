...
@pytest.mark.asyncio
async def test_begin_locked_turn_suppresses_superseded_revision_before_lock_side_effects(tmp_path: Path) -> None:
    """A stale edit revision must suppress before any under-lock side effect fires.

    ``commit_current_revision_under_lock`` returning ``False`` means a newer
    revision of this turn already committed under the same lifecycle lock. The
    currency check must run first, so the stale callback never invokes
    ``on_lifecycle_lock_acquired`` (which prunes the newer run's persisted state)
    or ``prepare_source_turn``.
    """
    bot = _bot(tmp_path)
    target = _target(thread_id="$thread", reply_to_event_id="$event")
    envelope = _envelope(target, source_event_id="$event")
    ...
    lifecycle_lock_calls: list[str] = []
    source_preparation_calls: list[str] = []

    request = ResponseRequest(
        thread_history=[],
        prompt="stale edit body",
        user_id="@user:localhost",
        response_envelope=envelope,
        payload_preparation=_preparation(target, envelope),
        commit_current_revision_under_lock=lambda: False,
        on_lifecycle_lock_acquired=lambda: lifecycle_lock_calls.append("lock"),
        prepare_source_turn=lambda: source_preparation_calls.append("prepare") is None,
    )
    ...
    assert prepared_request is None
    # The stale callback must not touch any under-lock side effect.
    assert lifecycle_lock_calls == []
    assert source_preparation_calls == []
    delivery_gateway.send_text.assert_not_awaited()
    request_preparer.prepare.assert_not_awaited()


@pytest.mark.asyncio
async def test_begin_locked_turn_runs_current_revision_through_lock_side_effects(tmp_path: Path) -> None:
    """A current edit revision must proceed past the currency check into the lock body."""
    bot = _bot(tmp_path)
    target = _target(thread_id="$thread", reply_to_event_id="$event")
    envelope = _envelope(target, source_event_id="$event")
    delivery_gateway = MagicMock(spec=DeliveryGateway)
    ...
    request = ResponseRequest(
        thread_history=[],
        prompt="current edit body",
        user_id="@user:localhost",
        response_envelope=envelope,
        payload_preparation=_preparation(target, envelope),
        commit_current_revision_under_lock=lambda: True,
        on_lifecycle_lock_acquired=lambda: lifecycle_lock_calls.append("lock"),
    )
    ...
    # A current revision advances into the lock body and its side effects.
    assert lifecycle_lock_calls == ["lock"]
