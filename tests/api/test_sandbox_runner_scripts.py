...
def test_worker_retirement_endpoint_authenticates_and_removes_only_exact_state(
    runner_client: tuple[TestClient, Path],
) -> None:
    """The shared runner confirms exact persistent-state absence without touching another worker."""
    client, workspace = runner_client
    runtime_paths = sandbox_runner_module.app_runtime_paths(sandbox_runner_app)
    ordinary_key = "v1:test:shared:ordinary"
    ordinary = prepare_worker_request(
        worker_key=ordinary_key,
        tool_init_overrides={},
        runtime_paths=runtime_paths,
        runner_token=_TOKEN,
    )
    worker_root = workspace.parent

    unauthorized = client.post(
        "/api/sandbox-runner/workers/retire",
        json={"worker_key": _WORKER_KEY},
    )
    ...
    assert retired.json() == {"retired": True}
    assert retired_again.status_code == 200
    assert retired_again.json() == {"retired": True}
    assert worker_root.exists() is False
    assert ordinary.paths.root.is_dir()
