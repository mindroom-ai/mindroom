...
def _redaction_event() -> nio.RedactionEvent:
    ...
    event.event_id = "$redaction:example.org"
    event.redacts = EVENT_ID
    return event


def _cleanup(turn_store: MagicMock, conversation_cache: MagicMock, regenerate: AsyncMock) -> RedactedTurnCleanup:
    return RedactedTurnCleanup(
...
async def test_redaction_tombstones_before_updating_advisory_cache() -> None:
    ...
    cleanup = _cleanup(turn_store, conversation_cache, regenerate)
...
@pytest.mark.asyncio
async def test_redacted_current_edit_regenerates_after_cache_reversion() -> None:
    """The model replay must observe the source body exposed by the applied redaction."""
    ordering: list[str] = []
    turn_store = MagicMock()
    record = MagicMock()
    turn_store.source_for_revision_event_id.return_value = (record, "$original:example.org")
    turn_store.mark_source_redacted.side_effect = lambda event_id: ordering.append(f"tombstone:{event_id}")
    conversation_cache = MagicMock()
