...
from mindroom.tool_system.events import (
    ...
    tool_trace_events_from_visible_content,
)
...
def test_build_then_parse_round_trips_entries() -> None:
    """Trace payload builder and parser should preserve known entries exactly."""
    entries = [
        ToolTraceEntry(
    ...
    assert payload is not None
    assert tool_trace_events_from_visible_content(payload) == entries


@pytest.mark.parametrize("content", [None, {}, {"body": "hello"}, {_TOOL_TRACE_KEY: "not-a-dict"}])
def test_tool_trace_events_from_visible_content_returns_none_without_parseable_trace(
    content: object,
) -> None:
    """Parser should return None when trace metadata is absent or malformed."""
    assert tool_trace_events_from_visible_content(content) is None


def test_tool_trace_events_from_visible_content_parses_top_level_trace() -> None:
    """Parser should decode the canonical top-level trace payload."""
    content = {
        _TOOL_TRACE_KEY: {
            "version": 2,
            "events": [
    ...
    parsed = tool_trace_events_from_visible_content(content)
...
def test_tool_trace_events_from_visible_content_parses_nested_new_content_trace() -> None:
    """Parser should fall back to visible edit payloads nested under m.new_content."""
    content = {
        "m.new_content": {
            _TOOL_TRACE_KEY: {
                "version": 2,
...
def test_tool_trace_events_from_visible_content_falls_back_to_outer_trace() -> None:
    """Parser should use the outer trace when m.new_content is empty."""
    content = {
        "m.new_content": {},
        _TOOL_TRACE_KEY: {
            "version": 2,
...
def test_tool_trace_events_from_visible_content_prefers_nested_new_content_trace() -> None:
    """Visible edit content under m.new_content should take precedence over top-level content."""
    content = {
        _TOOL_TRACE_KEY: {
            "version": 2,
            "events": [
...
def test_tool_trace_events_from_visible_content_ignores_non_mapping_new_content() -> None:
    """Parser should ignore non-dict edit content without crashing."""
    assert tool_trace_events_from_visible_content({"m.new_content": "not a dict"}) is None


def test_tool_trace_events_from_visible_content_returns_none_for_empty_new_content() -> None:
    """Parser should return None when m.new_content is present but genuinely empty."""
    assert tool_trace_events_from_visible_content({"m.new_content": {}}) is None


def test_tool_trace_events_from_visible_content_skips_malformed_entries() -> None:
    """Parser should ignore malformed entries and keep valid ones."""
    content = {
        _TOOL_TRACE_KEY: {
            "version": 2,
            "events": [
