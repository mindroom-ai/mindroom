...
from mindroom.config.models import DefaultsConfig, ModelConfig, RouterConfig, StreamingConfig
...
async def test_stream_stops_live_edits_past_configured_ceiling(config: Config) -> None:
    ...
    with (
        ...
        await _run_stream(capped_config, scripted_stream())

    assert [op.kind for op in gateway.ops] == ["send", "edit"]
    assert gateway.ops[0].display_text == "Hello"
    assert gateway.ops[1].display_text == "Hello world again"
    assert gateway.ops[1].content[STREAM_STATUS_KEY] == STREAM_STATUS_COMPLETED
