...
def _use_expensive_deletes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(knowledge_collections_module, "_reclaim_clock", lambda: next(elapsed))
...
async def test_pre_index_candidate_reclaim_is_bounded_per_run(
    ...
    assert len(surviving) == len(abandoned) - 1


@pytest.mark.asyncio
async def test_post_publish_reclaim_is_bounded_per_run(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Publishing cannot hold a scheduler slot while it drains the whole backlog."""
    docs_path = tmp_path / "docs"
    _write_corpus(docs_path, 2)
    config = _config(tmp_path, docs_path)
    runtime_paths = runtime_paths_for(config)
    ...
    surviving = [name for name in abandoned if name in _FakeVectorDb.store]
    assert len(abandoned) - len(surviving), "a publishing refresh still has to reclaim some backlog"
    assert len(surviving) >= len(abandoned) - 2
...
def test_failed_reclaim_stays_deferred_and_does_not_bypass_budget(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    failure_mode: str,
) -> None:
    """A failed delete remains backlog, while its attempt still consumes this call's budget."""
    docs_path = tmp_path / "docs"
    _write_corpus(docs_path, 1)
    config = _config(tmp_path, docs_path)
    manager = _manager(config)
    abandoned = _seed_abandoned_candidates(config, 2)
    attempted: list[str] = []

    def _fail_delete(vector_db: _FakeVectorDb) -> bool:
        attempted.append(vector_db.collection_name)
        if failure_mode == "raise":
            msg = "delete failed"
            raise RuntimeError(msg)
        return False

    monkeypatch.setattr(_FakeVectorDb, "delete", _fail_delete)
    _use_expensive_deletes(monkeypatch)

    with capture_logs() as logs:
        knowledge_collections_module.cleanup_superseded_collections(
            manager._collections,
            vector_db=manager._knowledge.vector_db,
            preserved=frozenset(),
            candidates_only=True,
            budget_seconds=30.0,
        )

    assert attempted == [abandoned[0]]
    assert all(name in _FakeVectorDb.store for name in abandoned)
    deferred_log = next(
        entry for entry in logs if entry["event"] == "Deferred superseded knowledge collections to a later refresh"
    )
    assert deferred_log["reclaimed"] == 0
    assert deferred_log["deferred"] == len(abandoned)
