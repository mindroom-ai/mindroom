...
def test_local_backend_fsyncs_staged_retirement_identity_before_canonical_unlink(
    ...
    events: list[str] = []
    original_fsync = worker_retirement_module.os.fsync
    original_unlink = worker_retirement_module.os.unlink
    ...
    monkeypatch.setattr(worker_retirement_module.os, "fsync", track_fsync)
    monkeypatch.setattr(worker_retirement_module.os, "unlink", track_unlink)
...
def test_local_backend_fsyncs_adopted_retirement_identity_before_canonical_unlink(
    ...
    with monkeypatch.context() as staging_fault:
        staging_fault.setattr(worker_retirement_module.os, "unlink", interrupt_after_staging)
...
def test_local_backend_fsyncs_adopted_retirement_identity_name_before_canonical_unlink(
    ...
    with monkeypatch.context() as staging_fault:
        staging_fault.setattr(worker_retirement_module.os, "fsync", interrupt_first_parent_sync)
...
def test_local_backend_binds_adopted_identity_validation_and_sync_for_fresh_retry(
    ...
    original_open = worker_retirement_module.os.open
    original_rmdir = worker_retirement_module.os.rmdir
    sidecar_opens = 0
    fail_root_once = True
    ...
    with monkeypatch.context() as retirement_faults:
        retirement_faults.setattr(worker_retirement_module.os, "open", mutate_same_inode_before_second_open)
        retirement_faults.setattr(worker_retirement_module.os, "rmdir", fail_first_worker_root_rmdir)
...
def test_local_backend_closes_worker_fd_when_open_validation_fails(
    ...
    state_root = backend.worker_root / worker_dir_name(run_key)
    state_root.mkdir()
    original_fstat = worker_retirement_module.os.fstat
    ...
    monkeypatch.setattr(worker_retirement_module.os, "fstat", fail_worker_fstat)
    fd_root = Path("/proc/self/fd")
    descriptors_before = len(tuple(fd_root.iterdir()))
...
def test_local_backend_late_root_failure_retries_without_reconstructing_identity(
    ...
    original_open = worker_retirement_module.os.open
    original_rmdir = worker_retirement_module.os.rmdir
    original_write = worker_retirement_module.os.write
    fail_root_once = True
    ...
    with monkeypatch.context() as retirement_faults:
        retirement_faults.setattr(worker_retirement_module.os, "open", fail_identity_recreation)
        retirement_faults.setattr(worker_retirement_module.os, "rmdir", fail_first_worker_root_rmdir)
        retirement_faults.setattr(worker_retirement_module.os, "write", fail_identity_rewrite)
        with pytest.raises(WorkerBackendError, match="final worker-root removal failure"):
            backend.retire_worker(run_key)
