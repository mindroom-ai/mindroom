...
import mindroom.bot as bot_module
...
class TestBotSyncLifecycle(ThreadingBehaviorTestBase):
    ...
    @pytest.mark.asyncio
    async def test_sync_forever_exact_canary_agent_enters_real_durable_runner(
        self,
        bot: AgentBot,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """The exact selected agent should traverse the real durable runner."""

        class DurableLoopStopped(RuntimeError):
            pass
        ...
        class Session:
            ...
            def next_batch(self, **limits: object) -> None:
                self.next_calls.append(limits)
                return None

        client = client_session._MindRoomAsyncClient("https://example.org")
        client.user_id = bot.agent_user.user_id
        client.device_id = "CANARYDEVICE"
        client.access_token = "access-token"
        bot.client = client
        room_id = bot.rooms[0]
        stream_id = UUID("77777777-7777-4777-8777-777777777777")
        bootstrap = MagicMock(stream_id=stream_id)
        session = Session()
        store_calls: list[tuple[object, dict[str, object]]] = []
        ingestion_calls: list[tuple[object, object, dict[str, object]]] = []
        sleep_calls: list[float] = []
        stopped = DurableLoopStopped("durable-loop-stopped")

        def open_store(path: object, **kwargs: object) -> object:
            assert path == bot.runtime_paths.storage_root / "tracking" / "nio_ingestion"
            assert path.is_dir()  # type: ignore[union-attr]
            store_calls.append((path, kwargs))
            return bootstrap
        ...
        def open_ingestion(
            ...
            **kwargs: object,
            ...
            ingestion_calls.append((selected_client, selected_bootstrap, kwargs))
            return session

        async def stop_after_idle(seconds: float) -> None:
            sleep_calls.append(seconds)
            assert session.run_started.is_set()
            raise stopped

        monkeypatch.setenv("MINDROOM_INGESTION_CANARY_AGENT", bot.agent_name)
        monkeypatch.delenv("MINDROOM_INGESTION_CANARY_STOP", raising=False)
        monkeypatch.delenv("MINDROOM_INGESTION_CANARY_TRACE", raising=False)
        legacy_sync = AsyncMock(side_effect=AssertionError("legacy sync selected"))
        try:
            with (
                patch.object(bot_module, "run_matrix_sync_forever", legacy_sync),
                patch.object(
                    durable_ingestion_module.nio_store,
                    "open_ingestion_store",
                    side_effect=open_store,
                ),
                patch.object(
                    durable_ingestion_module.nio,
                    "open_ingestion",
                    side_effect=open_ingestion,
                ),
                patch.object(
                    durable_ingestion_module.asyncio,
                    "sleep",
                    side_effect=stop_after_idle,
                ),
        ...
        assert raised.value.exceptions == (stopped,)
        legacy_sync.assert_not_awaited()
        assert len(store_calls) == len(ingestion_calls) == 1
        store_path, store_kwargs = store_calls[0]
        assert (
            store_path == bot.runtime_paths.storage_root / "tracking" / "nio_ingestion"
        )
        assert ingestion_calls[0][0] is client
        assert ingestion_calls[0][1] is bootstrap
        assert (
            ingestion_calls[0][2]["consumer_generation"]
            == store_kwargs["consumer_generation"]
        )
        assert ingestion_calls[0][2]["stream_id"] == stream_id
        assert ingestion_calls[0][2]["room_id"] == room_id
        assert session.next_calls == [{"max_records": 1}]
        assert sleep_calls == [0.05]
        assert session.run_cancelled is True
        assert session.enter_calls == session.exit_calls == 1
        bootstrap.close.assert_called_once_with()
        persisted = await bot._journal_principal().load_or_create_ingestion_consumer(
            new_generation=uuid4(),
        )
        assert persisted.generation == store_kwargs["consumer_generation"]
        assert persisted.stream_id == stream_id

    @pytest.mark.parametrize("selected", [None, "different-agent"])
    @pytest.mark.asyncio
    async def test_sync_forever_unset_or_mismatched_canary_is_exact_legacy_path(
        self,
        bot: AgentBot,
        monkeypatch: pytest.MonkeyPatch,
        selected: str | None,
    ) -> None:
        """Unset and mismatched selectors should retain the exact legacy call."""
        client = nio.AsyncClient("https://example.org")
        client.user_id = bot.agent_user.user_id
        client.device_id = "LEGACYDEVICE"
        client.access_token = "access-token"
        bot.client = client
        bot._first_sync_done = True
        bot._classic_sync_rebuild_pending = False
        nio_parent = bot.runtime_paths.storage_root / "tracking" / "nio_ingestion"
        assert not nio_parent.exists()
        if selected is None:
            monkeypatch.delenv("MINDROOM_INGESTION_CANARY_AGENT", raising=False)
        else:
            monkeypatch.setenv("MINDROOM_INGESTION_CANARY_AGENT", selected)
        monkeypatch.delenv("MINDROOM_INGESTION_CANARY_STOP", raising=False)
        monkeypatch.delenv("MINDROOM_INGESTION_CANARY_TRACE", raising=False)
        ...
        try:
            with (
                ...
                patch.object(
                    durable_ingestion_module.nio_store,
                    "open_ingestion_store",
                    side_effect=AssertionError("durable store opened"),
                ) as durable_store,
                patch.object(
                    durable_ingestion_module.nio,
                    "open_ingestion",
                    side_effect=AssertionError("durable nio session opened"),
                ) as durable_session,
                patch.object(
                    durable_ingestion_module.os,
                    "open",
                    side_effect=AssertionError("canary trace opened"),
                ) as trace_open,
            ):
                await bot.sync_forever()
        finally:
        ...
        legacy_sync.assert_awaited_once()
        call = legacy_sync.await_args
        ...
        assert call.kwargs["config"] is bot.config
        assert call.kwargs["room_ids"] is bot.rooms
        assert call.kwargs["sync_filter"] is bot_module._SYNC_FILTER
        durable_store.assert_not_called()
        durable_session.assert_not_called()
        trace_open.assert_not_called()
        assert not nio_parent.exists()
