...
def _migrated_config_init_mind_memory_config(knowledge_path: str) -> str:
    old_config = _old_config_init_mind_memory_config(knowledge_path)
    return (
        old_config.replace(
            """\
    knowledge_bases:
...
class TestConfigMigrate:
    ...
    def test_migrate_updates_old_config_init_mind_memory_without_reformatting(
        ...
        assert cfg.read_text(encoding="utf-8") == _migrated_config_init_mind_memory_config(knowledge_path)
