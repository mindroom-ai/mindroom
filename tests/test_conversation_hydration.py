...
class TestSidecarResolution:
    ...
    async def test_a_nested_edit_sidecar_repairs_the_current_revision(
        self,
        alice: PrincipalStore,
    ) -> None:
        ...
        nested = self._sidecar_source("$unused", "inner preview", "mxc://s/inner")["content"]
        complete = {"msgtype": "m.text", "body": "complete edited body"}
        await admit_all(alice, [original, edit])
        client = FakeClient(
            events={"$message": original},
            relations={"$message": [edit]},
            sidecars={
                "mxc://s/outer": json.dumps(
                    {"msgtype": "m.text", "body": "* inner preview", "m.new_content": nested},
                ),
                "mxc://s/inner": json.dumps(
                    {
                        "msgtype": "m.text",
                        "body": "* complete edited body",
                        "m.new_content": complete,
        ...
        assert [message.revision_event_id for message in page.messages] == ["$edit"]
        assert page.refresh_pending == ()
        await reader.read_strict(room_id=ROOM, thread_id=None, limit=10)
        assert client.downloads == ["mxc://s/outer", "mxc://s/inner"]

    @pytest.mark.parametrize("failure", ["cycle", "depth", "missing", "invalid", "oversized"])
    async def test_an_unresolved_nested_sidecar_keeps_the_read_incomplete(
        self,
        alice: PrincipalStore,
        failure: str,
    ) -> None:
        """Nested resolution stays bounded and never installs an incomplete preview."""
        source = self._sidecar_source("$long", "outer preview", "mxc://s/outer")
        next_url = "mxc://s/outer" if failure == "cycle" else "mxc://s/inner"
        nested = self._sidecar_source("$unused", "inner preview", next_url)["content"]
        sidecars = {
            "mxc://s/outer": json.dumps(
                {"msgtype": "m.text", "body": "* inner preview", "m.new_content": nested},
            ),
        ...
        if failure == "depth":
            third = self._sidecar_source("$unused", "third preview", "mxc://s/third")["content"]
            sidecars["mxc://s/inner"] = json.dumps({"m.new_content": third})
            sidecars["mxc://s/third"] = self._payload("beyond the resolution bound")
        elif failure == "invalid":
            sidecars["mxc://s/inner"] = "not JSON"
        elif failure == "oversized":
            sidecars["mxc://s/inner"] = self._payload("x" * (2 * 1024 * 1024))
        await admit_all(alice, [source])
        client = FakeClient(events={"$long": source}, sidecars=sidecars)
        reader = await self._reader(alice, client)
        ...
        assert page.messages == ()
        assert [request.logical_event_id for request in page.refresh_pending] == ["$long"]
        assert client.downloads == (["mxc://s/outer"] if failure == "cycle" else ["mxc://s/outer", "mxc://s/inner"])
