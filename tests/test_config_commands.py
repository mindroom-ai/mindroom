...
@pytest.mark.asyncio
async def test_handle_config_command_rejects_parent_of_privileged_config_prefix(tmp_path: Path) -> None:
    """Chat config should not expose a parent block containing restricted children."""
    config_path = tmp_path / "runtime-config.yaml"
    config_path.write_text(
        yaml.safe_dump(
            {
                ...
                "router": {"model": "default"},
                "agents": {"assistant": {"display_name": "Assistant", "role": "test"}},
                "memory": {"backend": "none", "embedder": {"provider": "openai", "id": "secret-embedder"}},
            },
        ),
        encoding="utf-8",
    )

    response, change_info = await handle_config_command("get memory", _runtime_paths_for_config(config_path))

    assert change_info is None
    assert "`memory` is restricted" in response
    assert "secret-embedder" not in response
