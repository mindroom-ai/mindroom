...
REG_TOKEN = os.environ.get("MINDROOM_REG_TOKEN", "").strip()
...
async def main() -> int:  # noqa: C901, PLR0915
    ...
    caller_id = os.environ["CALLER_USER_ID"]
    caller_dev = os.environ["CALLER_DEVICE_ID"]
    caller_tok = os.environ["CALLER_TOKEN"]
    bot_id = os.environ["BOT_USER_ID"]
    bot_dev = os.environ["BOT_DEVICE_ID"]
    ...
    from mindroom.credentials_sync import sync_env_to_credentials  # noqa: PLC0415
    ...
    from mindroom.config.auth import AuthorizationConfig  # noqa: PLC0415

    config = Config(
        authorization=AuthorizationConfig(global_users=[caller_id]),
        agents={
            AGENT: AgentConfig(
    ...
    try:
        ...
        for rp in caller_room.remote_participants.values():
            pubs = [(p.sid, str(p.kind), str(p.source), p.subscribed, p.muted) for p in rp.track_publications.values()]
            log(f"  [diag] caller sees {rp.identity}: pubs={pubs}")
        caller_pubs = [(p.sid, str(p.kind)) for p in caller_room.local_participant.track_publications.values()]
        log(f"  [diag] caller local pubs={caller_pubs}")
        session_obj = manager._sessions.get(room_id)
        bridge = session_obj.deps.bridge if session_obj else None
        broom = getattr(bridge, "_room", None)
        if broom is None:
            log("  [diag] bot bridge room is None!")
        else:
            bot_pubs = [(p.sid, str(p.kind)) for p in broom.local_participant.track_publications.values()]
            log(f"  [diag] bot identity={broom.local_participant.identity} local pubs={bot_pubs}")
            for rp in broom.remote_participants.values():
                pubs = [(p.sid, str(p.kind), str(p.source), p.subscribed) for p in rp.track_publications.values()]
                log(f"  [diag] bot sees {rp.identity}: pubs={pubs}")
            log(f"  [diag] bot roster={sorted(getattr(bridge, '_participant_identities', []) or [])}")
