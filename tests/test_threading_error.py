...
class TestThreadingBehavior:
    ...
    @pytest.mark.asyncio
    async def test_extract_dispatch_context_applies_reply_chain_replacement_relations(self, bot: AgentBot) -> None:
        """Dispatch snapshots should use the latest visible reply-chain edit state."""
        room = MagicMock(spec=nio.MatrixRoom)
        room.room_id = "!test:localhost"
        room.name = "Test Room"
        ...
        def room_get_event_relations(
            ...
            *,
                    ...
                    nio.RoomMessageText.from_dict(
                        {
                            "content": {
                                ...
                                "msgtype": "m.text",
                                "m.new_content": {
                                    "body": "Updated second message for @mindroom_general",
                                    "msgtype": "m.text",
                                    "m.relates_to": {"m.in_reply_to": {"event_id": "$msg1:localhost"}},
                                },
                                "m.relates_to": {
            ...
            async def iterator() -> object:
                for related_event in relations_by_event_id.get(event_id, []):
                    yield related_event
        ...
        assert context.thread_history[-1]["body"] == "Updated second message for @mindroom_general"
        assert context.thread_history[-1]["content"]["body"] == "Updated second message for @mindroom_general"
    ...
    @pytest.mark.asyncio
    async def test_extract_dispatch_context_preserves_cached_reply_chain_replacement_state_on_refresh_miss(
        self,
        bot: AgentBot,
    ) -> None:
        """Cached reply-chain edits should survive refreshes that fail to return a replacement."""
        room = MagicMock(spec=nio.MatrixRoom)
        room.room_id = "!test:localhost"
        room.name = "Test Room"
        ...
        assert first_context.thread_history[-1]["body"] == "Updated second message after cache fill"
        assert first_context.thread_history[-1]["latest_event_id"] == "$edit_msg2_v2:localhost"
        assert second_context.thread_history[-1]["body"] == "Updated second message after cache fill"
        assert second_context.thread_history[-1]["content"]["body"] == "Updated second message after cache fill"
        assert second_context.thread_history[-1]["latest_event_id"] == "$edit_msg2_v2:localhost"
        assert bot.client.room_get_event.await_count == 2
