...
class TestMatrixConversationCacheThreadReads:
    ...
    def test_notify_outbound_redaction_swallows_internal_write_failure(self, error: BaseException) -> None:
        ...
        access._outbound._schedule_fail_open_room_preflight = Mock(side_effect=error)
    ...
    @pytest.mark.asyncio
    async def test_notify_outbound_plain_reply_lookup_uses_resolved_thread_barrier(self) -> None:
        """Lookup-resolved plain replies should bypass sibling-thread barriers."""
        room_id = "!room:localhost"
        thread_a_id = "$thread-a:localhost"
        thread_b_id = "$thread-b:localhost"
        target_event_id = "$thread-reply:localhost"
        coordinator = _runtime_write_coordinator()
        event_cache = _runtime_event_cache()
        event_cache.get_thread_id_for_event = AsyncMock(
            side_effect=lambda lookup_room_id, event_id: (
                thread_a_id if (lookup_room_id, event_id) == (room_id, target_event_id) else None
            ),
        )
        client = _make_client_mock(user_id="@agent:localhost")
        access = MatrixConversationCache(
        ...
        async def mark_thread_stale(marked_room_id: str, thread_id: str, *, reason: str) -> None:
            assert marked_room_id == room_id
            assert thread_id == thread_a_id
            assert reason == "outbound_thread_mutation"
            thread_invalidation_started.set()
        ...
        access.notify_outbound_message(
            ...
            "$plain-reply:localhost",
            {
                "body": "bridged reply",
                "msgtype": "m.text",
                "m.relates_to": {"m.in_reply_to": {"event_id": target_event_id}},
            },
        )
        try:
            await asyncio.wait_for(thread_invalidation_started.wait(), timeout=1.0)
            await asyncio.wait_for(coordinator.wait_for_thread_idle(room_id, thread_a_id), timeout=1.0)
            assert sibling_thread_task.done() is False
        finally:
            release_sibling_thread_update.set()
            await asyncio.wait_for(
        ...
        event_cache.append_event.assert_awaited_once()
        append_args = event_cache.append_event.await_args.args
        assert append_args[0] == room_id
        assert append_args[1] == thread_a_id
        assert append_args[2]["event_id"] == "$plain-reply:localhost"

    @pytest.mark.asyncio
    async def test_notify_outbound_reference_lookup_uses_resolved_thread_barrier(self) -> None:
        """Lookup-resolved references should bypass sibling-thread barriers."""
        room_id = "!room:localhost"
        thread_a_id = "$thread-a:localhost"
        thread_b_id = "$thread-b:localhost"
        target_event_id = "$thread-reply:localhost"
        ...
        access.notify_outbound_message(
            ...
            "$reference:localhost",
            {
                "body": "reference",
                "msgtype": "m.text",
                "m.relates_to": {"rel_type": "m.reference", "event_id": target_event_id},
            },
        )
        try:
            await asyncio.wait_for(thread_invalidation_started.wait(), timeout=1.0)
        ...
        assert append_args[0] == room_id
        assert append_args[1] == thread_a_id
        assert append_args[2]["event_id"] == "$reference:localhost"

    @pytest.mark.asyncio
    async def test_notify_outbound_redaction_lookup_uses_resolved_thread_barrier(self) -> None:
        """Lookup-resolved redactions should bypass sibling-thread barriers."""
        room_id = "!room:localhost"
        thread_a_id = "$thread-a:localhost"
        thread_b_id = "$thread-b:localhost"
        redacted_event_id = "$thread-message:localhost"
        coordinator = _runtime_write_coordinator()
        event_cache = _runtime_event_cache()
        event_cache.get_thread_id_for_event = AsyncMock(
            side_effect=lambda lookup_room_id, event_id: (
                thread_a_id if (lookup_room_id, event_id) == (room_id, redacted_event_id) else None
            ),
        )
        event_cache.redact_event = AsyncMock(return_value=True)
        client = _make_client_mock(user_id="@agent:localhost")
        ...
        async def mark_thread_stale(marked_room_id: str, thread_id: str, *, reason: str) -> None:
            assert marked_room_id == room_id
            assert thread_id == thread_a_id
            assert reason == "outbound_redaction"
            thread_invalidation_started.set()
        ...
        access.notify_outbound_redaction(room_id, redacted_event_id)
        try:
            await asyncio.wait_for(thread_invalidation_started.wait(), timeout=1.0)
            await asyncio.wait_for(coordinator.wait_for_thread_idle(room_id, thread_a_id), timeout=1.0)
            assert sibling_thread_task.done() is False
        ...
        event_cache.redact_event.assert_awaited_once_with(room_id, redacted_event_id)

    @pytest.mark.asyncio
    async def test_notify_outbound_event_resolver_disagreement_uses_resolved_thread_barrier(self) -> None:
        """Claimed thread IDs must not pin outbound mutations to the wrong thread barrier."""
        room_id = "!room:localhost"
        claimed_thread_id = "$claimed-thread:localhost"
        ...
        claimed_thread_update_started = asyncio.Event()
        release_claimed_thread_update = asyncio.Event()
        thread_invalidation_started = asyncio.Event()

        async def blocking_claimed_thread_update() -> None:
            claimed_thread_update_started.set()
            await release_claimed_thread_update.wait()

        async def mark_thread_stale(marked_room_id: str, thread_id: str, *, reason: str) -> None:
            assert marked_room_id == room_id
            assert thread_id == resolved_thread_id
            assert reason == "outbound_thread_mutation"
            thread_invalidation_started.set()

        access._outbound._resolver.resolve_thread_impact_for_mutation = AsyncMock(
            return_value=MutationThreadImpact.threaded(resolved_thread_id),
        )
        event_cache.mark_thread_stale = AsyncMock(side_effect=mark_thread_stale)
        claimed_thread_task = coordinator.queue_thread_update(
            room_id,
            claimed_thread_id,
            blocking_claimed_thread_update,
            name="matrix_cache_blocking_claimed_thread_update",
        )
        await asyncio.wait_for(claimed_thread_update_started.wait(), timeout=1.0)
        ...
        access.notify_outbound_message(
            ...
            "$reply:localhost",
            {
                "body": "reply",
                "msgtype": "m.text",
                "m.relates_to": {"rel_type": "m.thread", "event_id": claimed_thread_id},
            },
        )
        try:
            await asyncio.wait_for(thread_invalidation_started.wait(), timeout=1.0)
            await asyncio.wait_for(coordinator.wait_for_thread_idle(room_id, resolved_thread_id), timeout=1.0)
            assert claimed_thread_task.done() is False
        finally:
            release_claimed_thread_update.set()
            await asyncio.wait_for(
                asyncio.gather(claimed_thread_task, return_exceptions=True),
                timeout=1.0,
            )
            await coordinator.wait_for_room_idle(room_id)
        ...
        append_args = event_cache.append_event.await_args.args
        assert append_args[0] == room_id
        assert append_args[1] == resolved_thread_id
        assert append_args[2]["event_id"] == "$reply:localhost"

    @pytest.mark.asyncio
    async def test_notify_outbound_event_unknown_resolution_uses_room_barrier_and_blocks_read(self) -> None:
        """Unknown outbound mutations should route through the room barrier, not the claimed thread."""
        room_id = "!room:localhost"
        claimed_thread_id = "$claimed-thread:localhost"
        sibling_thread_id = "$sibling-thread:localhost"
        coordinator = _runtime_write_coordinator()
        event_cache = _runtime_event_cache()
        client = _make_client_mock(user_id="@agent:localhost")
        access = MatrixConversationCache(
        ...
        async def mark_room_threads_stale(marked_room_id: str, *, reason: str) -> None:
            assert marked_room_id == room_id
            assert reason == "outbound_thread_lookup_unavailable"
            room_invalidation_started.set()
            await release_room_invalidation.wait()

        async def fetch_history(_room_id: str, _thread_id: str) -> ThreadHistoryResult:
            fetch_started.set()
            return thread_history_result(
                [_message(event_id=claimed_thread_id, body="Root")],
        ...
        try:
            ...
            read_task = asyncio.create_task(access.get_thread_history(room_id, claimed_thread_id))
            with pytest.raises(asyncio.TimeoutError):
