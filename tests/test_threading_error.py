...
class TestThreadingBehavior:
    ...
    @pytest.mark.asyncio
    async def test_room_cache_update_timing_logs_wait_and_run_breakdown_when_enabled(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Queued room updates should log predecessor wait versus update runtime when timing is enabled."""
        ...
        coordinator.queue_room_update(
            ...
            lambda: second_update(),
            name="matrix_cache_second_update",
            log_context={"thread_id": "$thread:localhost"},
        )
        ...
        timing_calls = [
            ...
            if call_args.args == ("Room cache update timing",)
            and call_args.kwargs.get("operation") == "matrix_cache_second_update"
        ]
        assert len(timing_calls) == 1
        timing_log = timing_calls[0].kwargs
        assert timing_log["room_id"] == "!test:localhost"
        assert timing_log["thread_id"] == "$thread:localhost"
        assert timing_log["queued_behind_predecessor"] is True
        assert timing_log["outcome"] == "ok"
        assert timing_log["predecessor_wait_ms"] > 0
        assert timing_log["update_run_ms"] > 0
        assert timing_log["total_ms"] >= timing_log["predecessor_wait_ms"]

    @pytest.mark.asyncio
    async def test_wait_for_room_idle_logs_initial_tail_wait_when_enabled(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Room-idle waits should log how long the caller blocked behind the current tail task."""
        monkeypatch.setenv("MINDROOM_TIMING", "1")
        ...
        idle_wait_task = asyncio.create_task(coordinator.wait_for_room_idle("!room:localhost"))
        await asyncio.sleep(0)
        release_blocker.set()
        ...
        assert len(timing_calls) == 1
        timing_log = timing_calls[0].kwargs
        assert timing_log["room_id"] == "!room:localhost"
        assert timing_log["waiting_for_operation"] == "matrix_cache_blocking_update"
        assert timing_log["wait_ms"] >= 0
