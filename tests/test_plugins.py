...
def test_load_plugins_registers_tools_and_skills(tmp_path: Path) -> None:
    ...
    try:
        ...
        tool = get_tool_by_name("demo_plugin", runtime_paths_for(config), execution_identity=None)
...
def test_load_plugins_from_python_package(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        tool = get_tool_by_name("demo_pkg_tool", runtime_paths_for(config), execution_identity=None)
