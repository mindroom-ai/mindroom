...
def build_queued_judgment_request(value: QueuedJudgmentInput) -> JudgmentRequest:
    ...
    return _complete_request(state_body, body)


def _complete_request(state_body: bytes, body: bytes) -> JudgmentRequest:
    return JudgmentRequest(
        model=PINNED_MODEL,
        body=body,
...
def build_participation_judgment_request(
    ...
    *,
    ...
    if len(body) > MAX_REQUEST_BYTES:
        return _incomplete("essential_input_too_large")
    return _complete_request(_canonical_json(state), body)
