...
def _is_standalone_admin_request(request: Request) -> bool:
    return _request_auth_user(request).get("user_id") == "standalone"


def _dashboard_matrix_requester_id(request: Request, runtime_paths: RuntimePaths) -> str | None:
    ...
    owner_user_id = runtime_paths.env_value(constants.OWNER_MATRIX_USER_ID_ENV)
    parsed_owner_user_id = try_parse_historical_matrix_user_id(owner_user_id)
    if parsed_owner_user_id is not None:
        return parsed_owner_user_id
...
def _authorized_api_room_requester_id(
    ...
    requester_id = _dashboard_matrix_requester_id(request, runtime_paths)
    if requester_id is None:
        if _is_standalone_admin_request(request):
            return None
