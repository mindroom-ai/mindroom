...
class MultiAgentOrchestrator:
    ...
    async def _run_knowledge_base_refresh(
        ...
        *,
        ...
        try:
            ...
            manager = self.knowledge_managers.get(base_id)
            if refreshed_manager is not None:
                self.knowledge_managers[base_id] = refreshed_manager
            elif manager is not None:
                self.knowledge_managers.pop(base_id, None)
            logger.info(
    ...
    def _schedule_knowledge_base_refresh(
        ...
        *,
        ...
        """Schedule refresh for one shared knowledge base without replacing other base refreshes."""
        task = self._knowledge_base_refresh_tasks.get(base_id)
        if task is not None and not task.done():
            return
        self._knowledge_base_refresh_tasks[base_id] = create_logged_task(
            self._run_knowledge_base_refresh(base_id, config, start_watcher=start_watcher),
