...
class _MultiAgentOrchestrator:
    ...
    def hook_matrix_admin(self) -> HookMatrixAdmin | None:
        ...
        return build_hook_matrix_admin(
            ...
            self.runtime_paths,
            config=self.config,
            on_created_room_persisted=router_bot.publish_persisted_invited_room,
        )
    ...
    async def reconcile_live_invites(self) -> None:
        """Recheck every live Matrix invite against current responder access."""
        await asyncio.gather(*(bot.reconcile_live_invites() for bot in self.agent_bots.values()))
