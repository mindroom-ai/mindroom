...
POSTGRES_DIALECT = SchemaDialect(
    placeholder="%s",
...
def _expand_byte_order(sql: str, dialect: SchemaDialect) -> str:
    """Spell the byte-order pin for one backend."""
    return sql.replace(_BYTE_ORDER_MARKER, dialect.order_by_bytes)


def schema_statements(dialect: SchemaDialect) -> tuple[str, ...]:
...
def render(sql: str, dialect: SchemaDialect) -> str:
