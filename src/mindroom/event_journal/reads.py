...
def _current_hydration(
    ...
    *,
    ...
    under is the room's current one. A marker from an earlier membership
    describes a relationship with the room that has already ended, so it is
    withheld and the conversation reads as unhydrated, which is what sends the
    next read to the server.
    """
    row = transaction.fetchone(
        """
