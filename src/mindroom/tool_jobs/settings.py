...
def pin_background_tool_jobs(config: Config, runtime_paths: RuntimePaths) -> bool:
    ...
    return _STARTED.setdefault(runtime_paths.storage_root.resolve(), config.background_tool_jobs)
...
def pending_background_tool_jobs_restart(config: Config, runtime_paths: RuntimePaths) -> bool:
    ...
    return background_tool_jobs_enabled(config, runtime_paths) != config.background_tool_jobs
