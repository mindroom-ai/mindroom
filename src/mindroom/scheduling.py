...
def _scheduled_task_signature_key_path(runtime_paths: RuntimePaths) -> Path:
    return runtime_paths.storage_root.joinpath(*_SCHEDULED_TASK_SIGNATURE_KEY_RELATIVE_PATH)
...
def _read_scheduled_task_signature_key(runtime_paths: RuntimePaths) -> bytes | None:
    ...
    try:
        ...
        msg = f"Invalid scheduled task signature key: {key_path}"
    ...
    if len(key) != _SCHEDULED_TASK_SIGNATURE_KEY_BYTES:
        ...
        msg = f"Invalid scheduled task signature key length: {key_path}"
        raise RuntimeError(msg)
    return key
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            ...
            try:
                ...
                logger.debug("scheduled_task_signature_key_fsync_unsupported", key_path=str(key_path))
    except OSError:
...
async def _persist_scheduled_task_state(
    ...
    if isinstance(response, nio.RoomPutStateResponse):
        return
    msg = f"Failed to persist scheduled task `{task_id}`: {response}"
    raise RuntimeError(msg)
    ...
    try:
        while True:
            ...
            with bound_log_context(**current_target.log_context):
                ...
                authorization_error = _scheduled_workflow_authorization_error(latest_workflow, config, runtime_paths)
                if authorization_error is not None:
                    logger.warning(
                        "cron_task_authorization_failed",
                        task_id=task_id,
                        room_id=latest_workflow.room_id,
                        created_by=latest_workflow.created_by,
                        reason=authorization_error,
                    )
                    await _save_one_time_task_status(client, latest_before_execute, "failed", runtime_paths)
                    return
...
async def schedule_task(  # noqa: C901, PLR0911, PLR0912, PLR0915
    ...
    if existing_task is None:
        existing_tasks = await get_scheduled_tasks_for_room(
            client=client,
            room_id=room_id,
            runtime_paths=runtime_paths,
            require_trusted=True,
...
async def cancel_scheduled_task(
    ...
    await _persist_scheduled_task_state(
        ...
        room_id=room_id,
        task_id=task_id,
        workflow=existing_task.workflow,
        runtime_paths=runtime_paths,
        status="cancelled",
        created_at=existing_task.created_at,
    )
