...
#: Seam for tests to drive reclaim deadlines without waiting for wall time.
...
def _partition_superseded_collections(
    ...
    *,
    ...
    """Split listed collections into owned superseded names and unowned names."""
...
def cleanup_superseded_collections(
    ...
    *,
    ...
    ``budget_seconds`` limits one sweep ahead of other refresh work. One delete
    is always attempted, even when that single operation exceeds the budget.
    Failed deletes remain deferred and still consume this sweep's budget.
    """
    if not isinstance(vector_db, ChromaDb):
        return
    ...
    superseded, unowned = _partition_superseded_collections(
        space,
        collection_names,
        preserved=preserved,
    ...
    for collection_name in superseded:
        if budget_seconds is not None and attempted and _reclaim_clock() - started_at >= budget_seconds:
            break
        attempted += 1
        try:
            deleted = _delete_collection_sync(space, collection_name)
        except Exception:
            logger.warning(
        ...
        if deleted:
            ...
            logger.warning(
                "Superseded knowledge collection still exists after cleanup failed",
                base_id=space.base_id,
                collection=collection_name,
            )
