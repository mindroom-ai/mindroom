...
#: Wall-clock slice for reclaiming superseded collections during one refresh.
#: Delete cost varies from microseconds to minutes across storage backends, so
#: elapsed time, not count, limits when more deletes start. Each sweep still
#: attempts one delete.
_SUPERSEDED_COLLECTION_RECLAIM_BUDGET_SECONDS = 30.0
...
class KnowledgeManager:
    ...
    async def _open_candidate_run(self, *, force_reindex: bool = False) -> _CandidateRun:
        ...
        # This runs ahead of the first indexed file, so it takes one bounded
        # slice and leaves the rest for later refreshes.
        if cleanup_is_safe:
            ...
            await asyncio.to_thread(
                ...
                budget_seconds=_SUPERSEDED_COLLECTION_RECLAIM_BUDGET_SECONDS,
            )
        else:
            logger.warning(
    ...
    async def reclaim_superseded_collections(self, *, published_collection: str | None) -> None:
        """Retire stale candidate state and reclaim one bounded collection slice.
        ...
        """
        checkpoint = await asyncio.to_thread(load_candidate_checkpoint, self._base_storage_path)
        preserved = {published_collection} if published_collection is not None else set()
        if checkpoint is not None and (
            checkpoint.collection == published_collection
            or await delete_collection(self._collections, checkpoint.collection)
        ):
            await asyncio.to_thread(delete_candidate_checkpoint, self._base_storage_path)
            logger.info(
            ...
            # One refresh gets one attempt at retiring the durable candidate.
            # Preserve it from the general sweep so a transient failure remains
            # observable and the next refresh owns the retry.
            preserved.add(checkpoint.collection)
