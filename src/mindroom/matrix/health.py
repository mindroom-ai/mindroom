...
MATRIX_SYNC_HEALTH_STALE_SECONDS = 180.0
...
class MatrixSyncHealthSnapshot:
...
def get_matrix_sync_health_snapshot(
    *,
    stale_after_seconds: float = MATRIX_SYNC_HEALTH_STALE_SECONDS,
    startup_grace_seconds: float = MATRIX_SYNC_STARTUP_GRACE_SECONDS,
    now: datetime | None = None,
) -> MatrixSyncHealthSnapshot:
    ...
    if not active_states:
        return MatrixSyncHealthSnapshot(
            active_entities=(),
            stale_entities=(),
            last_sync_time=None,
        )
