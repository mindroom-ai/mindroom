...
async def _create_postgres_event_cache_schema(db: AsyncConnection) -> None:
    ...
    await _create_postgres_cutover_triggers(db)


async def _create_postgres_cutover_triggers(db: AsyncConnection) -> None:
    """Keep version-2 cold storage safe while a version-1 runtime is still connected."""
    await db.execute(
        """
        CREATE OR REPLACE FUNCTION mindroom_event_cache_rehydrate_compacted_event()
        RETURNS TRIGGER AS $$
        BEGIN
            ...
            WHERE namespace = NEW.namespace AND event_id = NEW.event_id
    ...
    await db.execute(
        """
        CREATE OR REPLACE FUNCTION mindroom_event_cache_delete_redacted_compacted_events()
        RETURNS TRIGGER AS $$
        BEGIN
            DELETE FROM mindroom_event_cache_compacted_streaming_edits
            WHERE namespace = NEW.namespace
                AND room_id = NEW.room_id
                AND (event_id = NEW.event_id OR original_event_id = NEW.event_id);
            RETURN NEW;
        END;
        $$ LANGUAGE plpgsql
        """,
    ...
    await db.execute(
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1
                FROM pg_trigger
                WHERE tgname = 'mindroom_event_cache_rehydrate_compacted_event_trigger'
                    AND tgrelid = 'mindroom_event_cache_events'::regclass
            ) THEN
                EXECUTE 'CREATE TRIGGER mindroom_event_cache_rehydrate_compacted_event_trigger
                    AFTER INSERT OR UPDATE ON mindroom_event_cache_events
                    FOR EACH ROW EXECUTE FUNCTION mindroom_event_cache_rehydrate_compacted_event()';
            END IF;
        END
        $$
        """,
    ...
    await db.execute(
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1
                FROM pg_trigger
                WHERE tgname = 'mindroom_event_cache_delete_redacted_compacted_events_trigger'
                    AND tgrelid = 'mindroom_event_cache_redacted_events'::regclass
            ) THEN
                EXECUTE 'CREATE TRIGGER mindroom_event_cache_delete_redacted_compacted_events_trigger
                    AFTER INSERT ON mindroom_event_cache_redacted_events
                    FOR EACH ROW EXECUTE FUNCTION mindroom_event_cache_delete_redacted_compacted_events()';
            END IF;
        END
        $$
        """,
