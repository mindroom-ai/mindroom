...
class PersonalRoomService:
    ...
    async def _eligible_settings(
        ...
        if members is None:
            ...
            raise RuntimeError(msg)
        return settings if user_id in members else None
    ...
    async def ensure(self, user_id: str, source_room_id: str, source_client: nio.AsyncClient) -> str | None:
        ...
        async with async_exclusive_file_lock(path.with_suffix(".lock")):
            ...
            if record.room_id is None:
                record.room_id = await self._resolve_or_create(record, settings)
                await run_blocking_until_complete(write_personal_room, path, record)
            ...
            if not await self.change_membership(record.room_id, "join"):
                ...
                raise RuntimeError(msg)
            if roster.get(user_id) not in {"join", "invite"} and not await invite_to_room(
                self._client(),
                record.room_id,
                user_id,
            ):
    ...
    async def _resolve_or_create(self, record: PersonalRoomRecord, settings: PersonalRoomsConfig) -> str:
        client = self._client()
    ...
    async def _finish(
        ...
        *,
        ...
        assert record.room_id is not None
        client = self._client()
        if settings.requester_admin and not await ensure_room_admin_power_levels(
            client,
            record.room_id,
            [record.user_id],
        ):
    ...
    async def _set_avatar(self, record: PersonalRoomRecord, settings: PersonalRoomsConfig) -> None:
        assert record.room_id is not None
        client = self._client()
