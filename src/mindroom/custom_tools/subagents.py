...
class SubAgentsTools(Toolkit):
    ...
    def _context_error(tool_name: str) -> str:
        return SubAgentsTools._payload(
            tool_name,
            "error",
    ...
    @classmethod
    def _load_registry(cls, context: SessionToolsContext) -> dict[str, Any]:
    ...
    @classmethod
    def _save_registry(cls, context: SessionToolsContext, registry: dict[str, Any]) -> None:
        path = cls._registry_path(context)
    ...
    @classmethod
    def _update_all_runs_status(cls, context: SessionToolsContext, status: str) -> list[str]:
        with cls._registry_lock:
            registry = cls._load_registry(context)
            updated: list[str] = []
            now_iso = datetime.now(UTC).isoformat()
    ...
    @classmethod
    def _list_runs(cls, context: SessionToolsContext) -> list[dict[str, Any]]:
        with cls._registry_lock:
            registry = cls._load_registry(context)
    ...
    @classmethod
    def _registry_sessions(cls, context: SessionToolsContext) -> list[dict[str, Any]]:
        with cls._registry_lock:
            registry = cls._load_registry(context)
    ...
    def _read_agent_sessions(self, context: SessionToolsContext) -> list[dict[str, Any]]:
    ...
    async def _subagents_list_payload(self, context: SessionToolsContext, recent_minutes: int | None) -> str:
    ...
    async def _subagents_kill_payload(self, context: SessionToolsContext, target: str | None) -> str:
        if target is None:
