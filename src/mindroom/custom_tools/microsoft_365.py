...
DOCUMENT_CONTENT_KEY = "io.mindroom.document"
...
class _InvalidArgumentError(GraphError):
...
def _location(parent_reference: dict[str, Any]) -> str | None:
    """Return a readable folder path such as ``Finance / FY27`` from a parent reference."""
    path = parent_reference.get("path")
    if not isinstance(path, str) or ":" not in path:
        return None
    folders = [segment for segment in path.split(":", 1)[1].split("/") if segment]
...
class ConnectedDocument:
    ...
    @classmethod
    def from_item(cls, item: object) -> ConnectedDocument:
        """Return the document for a driveItem, accepting only Excel .xlsx files."""
        data = _mapping(item)
        parent = _mapping(data.get("parentReference"))
        name = data.get("name")
        ...
        if not isinstance(drive_id, str) or not isinstance(item_id, str):
            raise GraphError(code="invalid_response", message="Microsoft Graph did not identify the workbook's drive.")
        modified_by = _mapping(_mapping(data.get("lastModifiedBy")).get("user")).get("displayName")
        return cls(
...
def _card_body(event: str, document: ConnectedDocument, change: dict[str, object] | None) -> str:
...
def _name_candidates(name: str) -> list[str]:
    """Return ``Forecast.xlsx``, ``Forecast (2).xlsx``, ... for collision-free uploads."""
    path = PurePosixPath(name)
    match = _NUMBERED_NAME_PATTERN.match(path.stem)
    stem = match.group("stem") if match is not None else path.stem
    return [name, *(f"{stem} ({number}){path.suffix}" for number in range(2, _MAX_NAME_CANDIDATES + 1))]
...
class Microsoft365Tools(Toolkit):
    ...
    async def _post_card(
        ...
        body, metadata = document_card_content(
            event,
            document,
            requester_id=context.requester_id,
            agent_user_id=context.client.user_id if isinstance(context.client.user_id, str) else None,
    ...
    async def _document(self, token: str, ref: DocumentRef) -> ConnectedDocument:
        item = await graph_json(token, "GET", ref.path(), params={"$select": _ITEM_FIELDS})
        return ConnectedDocument.from_item(item)
    ...
    async def connect_office_document(self, url: str) -> str:
        ...
        async def connect(token: str) -> dict[str, object]:
            ...
            document = ConnectedDocument.from_item(item)
        ...
        if folder_share is not None:
            item = _mapping(
                await graph_json(
                    token,
                    "GET",
                    graph_path("shares", folder_share, "driveItem"),
                    params={"$select": "id,folder,parentReference"},
                ),
            )
            if item.get("folder") is None:
                raise GraphError(code="not_a_folder", message="folder_url must link to a folder.")
            return DocumentRef(drive_id=_mapping(item.get("parentReference")).get("driveId", ""), item_id=item["id"])
        try:
            item = _mapping(
                await graph_json(
                    token,
                    "GET",
                    f"{graph_path('me', 'drive', 'root')}:/{_DEFAULT_FOLDER}",
                    params={"$select": "id,folder,parentReference"},
                ),
            )
        except GraphError as exc:
    ...
    async def _upload_new(self, token: str, folder: DocumentRef, name: str, content: bytes) -> object:
        """Upload under the first free candidate name; Graph renames instead of replacing on a race."""
        for candidate in _name_candidates(name):
            child = f"{folder.path()}:{graph_path(candidate)}"
            try:
                await graph_json(token, "GET", child, params={"$select": "id"})
            except GraphError as exc:
                if exc.code != "not_found":
                    raise
            else:
                continue
            return await graph_upload(
                token,
                f"{child}:/content",
                content,
    ...
    async def read_office_document(self, document_id: str, range: str | None = None) -> str:  # noqa: A002
        ...
        async def read(token: str) -> dict[str, object]:
            ...
            document, outline = await asyncio.gather(self._document(token, ref), workbook_outline(token, ref))
            return {"document": document.summary(), "outline": outline}
    ...
    async def edit_office_document(
        ...
        numbers (0.12, not "12%") and set formats with number_format.
        ...
        Args:
            document_id: The document_id returned by connect_office_document or save_office_document.
            edits: Up to 25 edits covering at most 500 cells, each {"range": "Sheet!B4:B5", "before": [[0.08], [142]],
                "after": [[0.12], [150]], "number_format": [["0%"], ["0"]]}; number_format is optional.
        ...
        async def edit(token: str) -> dict[str, object]:
            ...
            result: dict[str, object] = _receipt_fields(receipt)
            if receipt.applied:
...
def _receipt_fields(receipt: EditReceipt) -> dict[str, object]:
    ...
    messages = {
        "applied": None,
        "partial": "Some edits were applied; check each edit's outcome.",
        "conflict": "Nothing was written because some ranges changed since they were read. "
        "Read them again and propose new edits.",
        "failed": "Nothing was written because the first write failed.",
    }
    if (message := messages[receipt.status]) is not None:
        fields.update(status="error", code=receipt.status, message=message)
    if receipt.status in {"applied", "partial"} and not receipt.verified:
        fields["warning"] = "Excel stored different content than was sent for at least one edit; see written."
    return fields
