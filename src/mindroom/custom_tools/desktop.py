...
from mindroom.desktop.protocol import MAX_COMMAND_TTL_MS, DesktopCommand, DesktopProtocolError
...
class DesktopTools(Toolkit):
    ...
    def __init__(
        ...
        register_toolkit_functions(
            ...
            descriptions={
                "desktop": (
                    "Observe or operate the configured local desktop. Start with status or screenshot. "
                    "Every control action returns a fresh screenshot; inspect it before the next action. "
    ...
    async def desktop(
        ...
        try:
            ...
            image_bytes = await download_encrypted_screenshot(context.client, response.screenshot)
            return ToolResult(
                content=content,
