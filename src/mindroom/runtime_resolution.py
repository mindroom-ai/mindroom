...
from mindroom.private_storage_upgrade import check_private_storage_target, check_runtime_storage_markers
...
def resolve_agent_storage(
    ...
    check_runtime_storage_markers(runtime_paths)
    ...
    if resolved_execution.policy.private_workspace_enabled:
        ...
        if worker_key is None:
            ...
            raise ValueError(msg)
        identity = resolved_execution.execution_identity
        if identity is not None and identity.requester_id is not None:
            check_private_storage_target(runtime_paths.storage_root, worker_key, identity.requester_id)
