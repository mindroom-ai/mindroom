...
def context_messages_from_visible_messages(
    ...
    *,
    ...
    for message in visible_messages:
        ...
        if max_message_length is not None:
            capped_body = _truncate_message_body(message.body, max_message_length)
            if not capped_body:
                continue
    ...
    *,
    ...
    current_prompt = (
        build_matrix_prompt_with_history(
            ...
            prompt_intro=current_prompt_intro,
...
def build_current_prompt_chain(
    ...
    *,
    ...
    messages = messages_with_current_prompt(
        ...
        current_prompt_intro=current_prompt_intro,
    )
...
def build_unseen_context_chain(
    ...
    *,
    ...
    in_progress_partial_reply_header: str = _IN_PROGRESS_PARTIAL_REPLY_HEADER,
    mixed_partial_reply_header: str = _MIXED_PARTIAL_REPLY_HEADER,
    history_header: str = "Previous conversation in this thread:",
    current_prompt_intro: str = "Current message:\n",
    render_messages_text_fn: Callable[[Sequence[Message]], str] = render_prepared_messages_text,
    estimated_tokens_fn: Callable[[str], int] | None = None,
    ...
    if partial_reply_kinds:
        context_messages = (
            Message(
                role="user",
                content=_build_unseen_messages_header(
                    partial_reply_kinds,
                    unseen_messages_header=unseen_messages_header,
                    interrupted_partial_reply_header=interrupted_partial_reply_header,
                    in_progress_partial_reply_header=in_progress_partial_reply_header,
                    mixed_partial_reply_header=mixed_partial_reply_header,
                ),
            ),
            *context_messages,
        )
