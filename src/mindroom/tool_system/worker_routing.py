...
def visible_state_roots_for_worker_key(
    ...
    *,
    ...
    `user_scope_agent_names` (the non-private agents that resolve to
    `worker_scope: user`) plus its own private-instance namespace, and never the
    roots of agents on other scopes.
    """
    scope = resolved_worker_key_scope(worker_key)
    if scope is None:
        return ()
