...
_TOKEN_LIKE_PATTERN = re.compile(
    r"(?<![A-Za-z0-9])(?P<token>(?:sk|pk)-[A-Za-z0-9._-]+|xox[baprs]-[A-Za-z0-9-]+)(?![A-Za-z0-9])",
)
...
def _is_secret_key(value: object) -> bool:
    normalized = _normalize_secret_key(str(value))
...
def sanitize_failure_value(value: object, *, depth: int = 0) -> JsonValue:
    ...
    return sanitize_failure_text(repr(value))
...
def _append_failure_record(record: ToolFailureRecord, runtime_paths: RuntimePaths) -> None:
    _failure_logger(_failure_log_path(runtime_paths)).info(json.dumps(record.as_dict(), sort_keys=True))
