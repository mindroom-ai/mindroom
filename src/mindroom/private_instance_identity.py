...
def load_private_instance_identity(
    ...
    raw_payload = _load_record_payload(trusted_scope_root / _RECORD_FILENAME)
...
def _load_record_payload(record_path: Path) -> object | None:
    """Read one regular record file without following a symlink."""
    try:
...
def _object_with_unique_keys(pairs: list[tuple[str, object]]) -> dict[str, object]:
    """Build one JSON object only when every member name appears once."""
...
def _trusted_scope_root(base_storage_path: Path, scope_root: Path, *, create: bool) -> Path:
    """Validate the direct private-scope directory beneath one trusted storage root."""
    ...
    namespace_exists = _validate_directory_entry(namespace_path, "private-instance namespace")
    if not namespace_exists:
        if not create:
            return candidate_scope_root
        namespace_path.mkdir(mode=0o700, parents=True, exist_ok=True)
        _validate_directory_entry(namespace_path, "private-instance namespace")

    scope_exists = _validate_directory_entry(candidate_scope_root, "private-instance scope root")
    if not scope_exists and create:
        candidate_scope_root.mkdir(mode=0o700)
...
def _validate_directory_entry(path: Path, name: str) -> bool:
    """Return whether a direct path entry exists as a non-symlink directory."""
    try:
...
def _validate_identity(
    identity: PrivateInstanceIdentity,
    base_storage_path: Path,
    scope_root: Path,
) -> None:
    ...
    expected_scope_root = private_instance_scope_root_path(base_storage_path, identity.worker_key)
    if scope_root.expanduser().absolute() != expected_scope_root:
...
def _raise_unreadable_record(error: BaseException) -> NoReturn:
    """Raise the contract error for filesystem and JSON read failures."""
