...
class ThreadInviteManager:
    ...
    async def cleanup_expired_invitations(self, room_id: str, timeout_hours: int = DEFAULT_TIMEOUT_HOURS) -> int:
        """Remove agents whose invitations have expired.

        Args:
            room_id: The room ID to clean up
            timeout_hours: Hours after invitation before removing agent
                ...
                if invited_at_str:
                    try:
                        ...
                        if now - invited_at > timeout_threshold:
                            ...
                            if ":" in state_key:
                                agent_name = state_key.split(":", 1)[1]
                                expired_invites.append((state_key, agent_name))
                    except (ValueError, TypeError):
                        pass

        # Remove expired agents from room and their invitations
        for state_key, agent_name in expired_invites:
            ...
            kick_response = await self.client.room_kick(
                ...
                reason="Thread invitation expired",
            )

            if isinstance(kick_response, nio.RoomKickResponse):
                logger.info(
                    "Removed expired agent from room",
                    agent=agent_name,
                    room_id=room_id,
                )
                ...
                logger.error(
                    "Failed to kick expired agent from room",
                    agent=agent_name,
                    room_id=room_id,
