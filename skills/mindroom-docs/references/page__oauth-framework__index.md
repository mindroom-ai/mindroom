...
The built-in Microsoft 365 provider uses the Microsoft identity platform v2 endpoints, stores tokens only in the requester's `user` scope, requests `offline_access` and `Files.ReadWrite.All` for [connected workbooks](https://docs.mindroom.chat/tools/microsoft-365/), reads its app client from `microsoft_365_oauth_client`, and takes its tenant from `MICROSOFT_365_TENANT_ID`.
