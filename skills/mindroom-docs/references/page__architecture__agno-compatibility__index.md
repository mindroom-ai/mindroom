...
| `tool_call_limit` refuses calls past the limit but keeps calling the model for as long as it requests tools. | End the run when the model requests tools again after its first refused batch, keeping that one closing response; tracked in [issue #8304](https://github.com/agno-agi/agno/issues/8304) and [issue #10041](https://github.com/agno-agi/agno/issues/10041), whose open PRs [#10042](https://github.com/agno-agi/agno/pull/10042) and [#8324](https://github.com/agno-agi/agno/pull/8324) stop without that response. | `agno_compat_model_hooks.py`; real Agent and Team loops in `tests/test_tool_call_budget.py`. |
...
| `agno_compat_model_hooks.py` | Model message projection, post-tool formatting/media, asynchronous post-batch checkpoint chaining, and resumed-response callbacks, the tool-call-limit run stop, permanent and scoped invocation/retry bindings, answer-cache bypass, and provider client factories. | Approval receipts, queued notices and mid-turn judgments, the tool-call budget warning, request logging, media fallback, provider stream retries, prompt caching, and participation decisions remain in their owning modules. |
