...
| Tool-hook argument injection reconstructs the same signature ten times per call. | Tracking gap: prepare stable hook signatures once while preserving argument injection and fresh hook ownership on reload. | The per-bridge signature in `tool_system/tool_hooks.py`; `tests/test_tool_hooks.py::test_agno_tool_calls_reuse_bridge_signature`. |
...
| `tool_system/tool_hooks.py` | Fixed signature metadata on each newly created owned hook bridge. | Hook execution and reload ownership remain here; plugin hook and tool entrypoint signatures are not cached. |
