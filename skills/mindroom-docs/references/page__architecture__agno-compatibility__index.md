...
| Closing an Agent stream between chunks can persist different run and session totals. | Close nested model streams before cancellation accounting and persistence; no matching upstream fix identified. | Separate from request-message retention: synthetic consumer closure reproduces a nonzero run ledger with an empty session aggregate even without the local repair. |
...
| `agno_compat_run_messages.py` | Version-guarded Agent/Team terminal snapshots and retention of metered messages from interrupted model streams. | `agent_storage.py` installs the patch; provider adapters own reported counters, and `usage_storage.py` retains content-free projection and retry exclusions. Aggregate accounting remains with Agno and the session-metrics repair. |
