...
| Run saves commit internally, with no caller-owned transaction or persistence hook. | Tracking gap: an in-transaction hook is needed to persist application-owned usage and compaction-archive restores atomically; no matching upstream issue or PR identified. | `agno_compat_sqlite.py` copies the Agno 3.0.9 run UPSERT; `usage_storage.py` owns the content-free schema and projection, and `history/archive_schema.py` owns the archive schema. |
...
| Response loops call the model for as long as it requests tools: `tool_call_limit` only refuses counted calls, and calls to unknown tools or with unparseable arguments bypass it. | End every loop after a bounded number of model requests through the normal completion path; refused `tool_call_limit` batches are tracked in [issue #8304](https://github.com/agno-agi/agno/issues/8304) and [issue #10041](https://github.com/agno-agi/agno/issues/10041), whose open PRs [#10042](https://github.com/agno-agi/agno/pull/10042) and [#8324](https://github.com/agno-agi/agno/pull/8324) stop only after limit refusals, and no upstream issue was identified for unknown-tool or unparseable-argument loops. | `agno_compat_model_hooks.py`; real Agent and Team loops in `tests/test_tool_call_budget.py`. |
...
| `agno_compat_sqlite.py` | Private pragma listener removal, private run-table access, monotonic run insertion inside owner transactions with atomic usage snapshots, atomic run/legacy deletion transaction, and private cache counters. | `agent_storage.py` retains journaling choice, prompt sanitization, descendant selection, diagnostics, and legacy scrub policy; `history/archive.py` shares the deletion and insertion transactions to move runs into and out of the compaction archive. |
