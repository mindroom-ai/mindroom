...
| --- | --- | --- |
| Session totals add a run's full cumulative metrics again on pause/resume and checkpoints, or are omitted for sessions without a `session_data` mapping. | Initialize missing session totals and make Agent and Team accumulation idempotent for repeated saves of one run, including member contributions. | `agno_compat_session_metrics.py`; real approval/resume, checkpoint, reload and export reconciliation tests. |
...
| `agno_compat_openai_responses.py` | Responses SDK stream ownership, completion, response-ID publication, native failure normalization, retry protection, continuation adaptation, and cache-write usage preservation. | `openai_models.py` retains request configuration and native/portable replay policy; `provider_stream_retry.py` owns retry limits and delays. |
