...
The sub-agent needs only bash and curl: when it runs the script, the callback message lands in the minting thread and wakes the orchestrating agent.

For the default one-use callback, an unused `on_expiry: notify` callback posts a timeout notice instead.

Callbacks with larger use budgets may wake the orchestrator multiple times and post an expiry notice when uses remain.
...
Omitting `ttl_seconds` from `mint_callback` uses `default_ttl_seconds`.
...
The generated URL defaults to `http://127.0.0.1:8765`.

Set `MINDROOM_URL` to an address reachable by the callback consumer when it runs outside the primary MindRoom host or container.
...
Records store `expires_at` and a dedicated periodic sweep collects expired records about once per minute.

An expired callback that still has unused fires and `on_expiry: notify` first posts `⏰ Callback '<label>' expired with N unused fire(s) (created <ts>)` into the target thread through the same delivery gates.
...
Delivery and cleanup span Matrix and local storage, so this retry path is at-least-once rather than a distributed exactly-once guarantee.
