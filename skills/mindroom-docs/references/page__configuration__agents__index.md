...
Use `job(action="list")` to rediscover jobs and `job(action="inspect"|"wait"|"resume"|"cancel", job_id=...)` to manage a turn; see [Background jobs](https://docs.mindroom.chat/tools/agent-orchestration/#background-jobs).
