...
Worker-owned cache and virtualenv env names are reasserted after the hook, so hooks cannot redirect `XDG_CACHE_HOME`, `PIP_CACHE_DIR`, `UV_CACHE_DIR`, `PYTHONPYCACHEPREFIX`, or `VIRTUAL_ENV`.
