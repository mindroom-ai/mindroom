...
| `max_tool_calls_per_turn` | int | `null` | Tool calls one turn may execute. Further calls return a tool error and the model gets one more response to answer; if it requests tools again, the turn ends with the text streamed so far (`null` = `defaults.max_tool_calls_per_turn`, 500) |
