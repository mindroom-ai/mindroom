...
agents:
  developer:
    ...
    # here or under defaults.compaction.
    compaction:
      enabled: true
...
| `compaction`                  | object | `null`      | Per-agent auto-compaction overrides. When no compaction block is authored, MindRoom still uses `context_window` as a default replay-overflow guard that reduces or disables replay for the current run without rewriting stored history. Authoring `defaults.compaction` or this per-agent block lets you customize or disable that behavior with `enabled`, `threshold_tokens`, `threshold_percent`, `reserve_tokens`, `model`, and `notify`. Replay budgeting and compaction require a usable `context_window` on the active model or the selected `compaction.model`. Compaction is destructive inside the live session: it merges old history into `session.summary` and removes the compacted raw runs from `session.runs`                                                                                                                                |
