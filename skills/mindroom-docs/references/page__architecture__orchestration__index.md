...
- **`src/mindroom/model_loading.py`** — Owns `get_model_instance()`, provider-specific model loader selection, and installation of provider-boundary request wrappers.
- **`src/mindroom/media_fallback.py`** — Owns the single retry without inline media when a provider rejects a media-bearing request.
