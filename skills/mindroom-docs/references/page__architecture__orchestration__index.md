...
- **Sync loops**: Each bot runs `sync_forever_with_restart()` with automatic retry; the default `matrix_sync.mode: sliding` uses MSC4186 Simplified Sliding Sync and `matrix_sync.mode: classic` uses classic `/v3/sync`
