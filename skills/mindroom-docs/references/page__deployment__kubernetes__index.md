...
> [!WARNING] The current Kubernetes worker backend mounts the shared storage root into each worker pod. That gives the worker access to canonical agent state, but it does not by itself create agent-level filesystem isolation. `user` creates one persistent runtime per requester. Multiple agents may run inside that runtime. Those agents may access each other's mounted files inside that runtime. Treat `worker_scope=user` as a per-requester workstation or trust-sharing mode. Use `user_agent` if you need the clearest per-agent filesystem isolation.
