...
Every finished response to a person, including one that failed or paused for approval, fixes where a new conversation's counting starts, so an approved continuation counts even when its request opened the conversation.
...
Older tool results are left out, and a very long message keeps its start and end, with private keys removed from the whole message and secrets redacted across each cut.
