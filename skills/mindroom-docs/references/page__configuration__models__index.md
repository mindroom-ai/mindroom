...
Summary-input chunk sizing counts with the model's tiktoken encoding only when the summary model provably targets the genuine OpenAI endpoint: provider `openai` with no custom `base_url`, no `client_params`, no prebuilt or custom HTTP client, and no `OPENAI_BASE_URL` override — any unrecognized client customization falls back conservatively.
...
The byte count is a proven token upper bound for byte-level BPE tokenizers, where every token consumes at least one byte; this is established for Claude and the o200k-family OpenAI encodings.
SentencePiece-based tokenizers such as Gemini's can normalize text before segmentation (NFKC-class), and rare compatibility characters (for example U+FDFA) can expand beyond their byte length there, so the bound is not proven for that class.
Realistic chat and tool content typically sits 3-4x below the bound, and the compaction budget's reserve and safety margin absorb such pockets.
Every persisted summary must also pass a fit check before it is written: sized by the same estimator the next request will use, its wrapped summary block must leave at least max(1,000 units, a quarter of the summary input budget) of headroom, and when `compaction.fallback_model` is configured the check covers both models, so persisted summaries fit the next compaction request by construction.
A generated summary that fails the fit check is rejected before persistence and retried once with smaller input; exhaustion fails that compaction attempt with nothing persisted.
A stored summary that predates this contract, or was written under a larger model or budget, gets one provider-arbitrated condensation attempt carrying the complete summary with no client-side size gate.
Condensed output that fits is persisted immediately as durable summary-only progress, and strictly smaller output that still fails the fit check is persisted together with a durable marker that stops further automatic attempts for that state.
A provider context-window rejection of that condensation — typed, or surfaced as a legacy context-length message — is terminal for that exact combination of summary, serving model profile (model and endpoint), and budget: MindRoom records a durable marker, spends nothing further on it, and reports the remedies (raise the compaction model's `context_window`, switch `compaction.model`, or force a retry with the `compact_context` tool).
