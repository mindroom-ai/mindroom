...
For clients that send plain replies without thread metadata (`m.in_reply_to` but no `rel_type: m.thread`), MindRoom treats them as plain replies. Only explicit `m.thread` metadata creates thread conversation context.
...
1. Keeps `m.in_reply_to` only as reply-target UX metadata.
1. Does not derive thread or session identity from plain reply chains.
