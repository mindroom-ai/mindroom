...
A room not yet verified for its key must also publish the alias as its canonical or alternative alias, which stops one key's alias from adopting another key's room.
A room not yet recorded for its key must also have no user outside the room's configured `admins` holding admin power (100) or, from room version 12, being a co-creator.
Later alias or admin drift in the recorded room is logged as `managed_room_policy_drift` instead, because the router still owns the room.
...
An existing Space must pass the same ownership check as managed rooms before MindRoom records it, links rooms into it, or invites anyone to it, and MindRoom never joins a Space it finds by alias.
Other Space admins do not disqualify it, because the Space grants no room access and earlier releases made room invitees Space admins.
