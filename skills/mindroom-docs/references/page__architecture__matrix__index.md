...
Each request reads current config. Models are sorted by stable key; the SHA-256 revision covers only published entries, including labels and resolved Matrix icons. Display names may repeat and fall back to the key. Icons use Matrix `mxc://` URIs only; local raster publication follows the [model configuration rules](https://docs.mindroom.chat/configuration/models/).
