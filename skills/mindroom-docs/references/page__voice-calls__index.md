...
If persisted invite state names multiple calls-enabled agents for one ad-hoc room, MindRoom refuses to choose by configuration order and reports the conflicting agents and room ID.
Assign that room ID under `agents.<name>.rooms` for exactly one calls-enabled agent to make ownership authoritative without weakening the other accounts' room-membership policy.
