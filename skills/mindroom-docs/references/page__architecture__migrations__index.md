...
| [`src/mindroom/history/legacy_compaction_state.py`][legacy-compaction-state] | History preparation or redaction cleanup finds v2 state carrying `compacted_run_ids` or last-compaction audit fields, or a replayed summary in a scope without archive generations. | `adopt_legacy_compaction` records one content-free legacy generation with the summary, its possible seen ids, and tombstones, and adopts reappearing keys again only when an older release deleted unarchived runs or cleared the summary; `history/storage.py` drops the retired keys and keeps replay, tombstone pruning, and legacy redaction invalidation. |
...
| [`history/legacy_compaction_state.py`][legacy-compaction-state] | [Legacy compaction tests][legacy-compaction-state-tests] cover recognition, one-time adoption, tombstone pruning, and summary-only sessions; [compaction redaction tests][compaction-redaction-tests] cover legacy invalidation. |
