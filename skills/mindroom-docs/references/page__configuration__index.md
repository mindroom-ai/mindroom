...
defaults:
  ...
  worker_scope: null               # Default: null (unscoped proxied execution unless an agent opts into reusable worker runtimes)
...
- `agents.<name>.context_files` inject workspace-relative file context into each freshly built agent instance, so edits affect the next reply/request in the normal runtime paths (see [Agents](https://docs.mindroom.chat/configuration/agents/index.md))
