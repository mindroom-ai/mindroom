...
MindRoom runs startup thread prewarm once per runtime, not once per bot. When a router exists, the router owns it. If no router is managed, ownership falls back to the first managed bot. A team-level `startup_thread_prewarm` flag only matters when that team is the selected owner.
