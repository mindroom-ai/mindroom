...
| `access` | No | `null` | Conversation-access policy with `current_room_members`, `members_of_rooms`, and `users`. Omitting it grants members of this team's own managed `rooms`. Requesters must also satisfy every member agent's own `access`. See [Authorization](https://docs.mindroom.chat/authorization/) |
