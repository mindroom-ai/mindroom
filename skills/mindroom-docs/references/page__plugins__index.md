...
| --- | --- | --- | --- |
| `name` | string | **yes** | Unique plugin identifier using lowercase ASCII letters, digits, `-`, and `_` only (pattern: `^[a-z0-9_-]+$`). |
...
Duplicate manifest names stop plugin loading in every mode.
`mindroom config validate` rejects missing, invalid, or malformed plugin entries.
Normal `mindroom run` startup skips plugin entries that cannot resolve or load, logs a warning, and disables their known tools for that runtime snapshot.
If a skipped plugin's tool names cannot be fully resolved, unknown authored tool references are also disabled with a warning that asks the operator to check for a typo.
...
- Plugin imports must not start background `asyncio.Task` objects because candidate modules are staged before activation; start them from an `@hook("agent:started")` callback and deduplicate stored tasks per entity instead.
...
Recovered plugin tools become available again after the next valid reload.
