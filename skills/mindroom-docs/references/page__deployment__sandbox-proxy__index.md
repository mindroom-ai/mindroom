...
- Runtimes that act for one requester (`user` and `user_agent` workers, background script runs, and CLI agent turns) only source a hook from a private agent's workspace or the worker's own scratch workspace.
  A hook in a non-private agent's `agents/<agent>/workspace` is writable by every requester's runtime for that agent, so these runtimes ignore it and log `workspace_env_hook_ignored_in_shared_workspace` once per workspace.
