...
In the intended model, add a shared agent-state volume or bind mount that both the primary runtime and the runner can access. Keep secrets and other primary-only files on separate mounts that are not exposed to the runner.
