...
`google_drive_upload_file()` uploads a local file, resolves relative paths from the agent workspace, and follows the agent's [`file_access`](https://docs.mindroom.chat/architecture/security-posture/#file-access) setting for which local files it may read.
