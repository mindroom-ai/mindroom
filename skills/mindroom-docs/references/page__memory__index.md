...
Semantic knowledge and file-memory indexes support the narrower [semantic embedder provider set](https://docs.mindroom.chat/knowledge/#embedder-configuration).
...
For `openai` and `anthropic`, an `api_key` in `memory.llm.config` is used as given; without one, MindRoom uses the provider's shared key.
...
The generated file-memory identity includes the effective memory root and scope; see [Knowledge storage](https://docs.mindroom.chat/knowledge/#storage).
...
The file backend skips symlinks, FIFOs, device nodes, and anything under a symlinked directory when it reads entries for the prompt preload, keyword search, and listing, and it rejects them when it writes, so an entry planted inside the scope directory cannot redirect MindRoom to another file.
