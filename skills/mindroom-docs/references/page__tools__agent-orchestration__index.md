...
The response adds `daily_breakdown`, with one row per UTC calendar date containing `date` (`YYYY-MM-DD`), token `totals`, and `run_count`.
...
It includes the same token counters as the other breakdowns, including cache, reasoning, and audio tokens.
