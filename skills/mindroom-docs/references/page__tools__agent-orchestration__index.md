...
- `self_config` blocks privileged self-escalation by rejecting privileged tools the agent does not already hold, including `config_manager`, code-execution and host-control tools (`shell`, `python`, `coding`, `script`, `file`, `claude_agent`, `docker`, `e2b`, `daytona`, `aws_lambda`, `desktop`, and the browser toolkits), low-level Matrix and arbitrary-request tools (`matrix_api`, `custom_api`, `composio`), database query tools, and platform-control tools (`scheduler`, `external_trigger_manager`, `callback_manager`, `invite_router`, `dynamic_workflow`, `agent_vault_access`, `approved_egress`, `oauth_connections`).
