...
- `src/mindroom/api/homeassistant_integration.py` and `src/mindroom/custom_tools/homeassistant.py`: validate Home Assistant URLs before outbound requests, require explicit private/local URL opt-in, and avoid surfacing upstream response bodies on failures.
...
- `tests/test_server_fetch_url_validation.py`, `tests/test_website_tool.py`, `tests/test_homeassistant_tools.py`, and `tests/api/test_api.py`: added focused coverage for private URLs, localhost, metadata-style targets, public URLs, private opt-in behavior, and redirect-to-private behavior.
