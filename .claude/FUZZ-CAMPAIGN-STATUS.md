...
Startup-batch fresh review (reviewer `prreview1639b`, range `e60601ff4..3eb1f597d`): ONE blocker B1 — `_finish_recovered_bot_start` called `resume_pending_recheck` without the `self.running` guard the reload caller has; a bot recovery landing in stop()'s window (running flipped, maintenance cancelled, retry tasks not yet cancelled, bots still marked running) could schedule a detached untracked recheck task against a closing client. VERIFIED against code and FIXED in `f4b4b3930` (mirror guard + shutdown-race regression `test_bot_recovery_mid_shutdown_does_not_schedule_pending_recheck`, 1 passed). All other focus areas verified clean by the reviewer: stamp reaches every nonterminal delivery path (all send/edit callers audited), prior-gen/foreign-bot cannot be wrongly protected (exact match + None guard), bounded-retry cancellation correct, regression quality strong. Reviewer had not yet seen `6118cd6aa` (per-delivery stamp move) — re-verify requested.

## Swarm blocker batch 2: dispatch/coalescing (OPEN — dispatch_races.md, verified pending)

- D1 blocker claim: early `is_claimed_in_flight` precheck (ingress_validation.py) has a TOCTOU window — a duplicate can pass precheck before the original claims, stall in resolution/lane wait, then co-batch with a fresh innocent source; the all-or-nothing `try_claim_turn` collision drops the batch and starves the innocent source. Reviewer probe confirmed the collision leaves `$b` unclaimed. Required root fix per reviewer: ownership filtering at gate admission or claim-time salvage of non-conflicting sources.
- D2 blocker claim: an edit accepted while the original response is in flight is permanently discarded (`edit_regenerator.py` sees `response_event_id is None`, logs `missing_previous_response_for_edit`, returns). MUST CLASSIFY pre-existing-on-main vs PR-introduced before treating as a PR #1639 gate.
- D3 risk claim: `_pending_claimed_event_ids` owns only the relay ID; aliases discovered during preparation (routed original) are not added to the claim, so replays addressed via the routed-original ID bypass `is_claimed_in_flight`.
