...
8. **Send**: native todo poke message ("Todo work is ready" style, list up to 5 items with priorities) from the router into the stored room/thread with explicit assignee mention, `trigger_dispatch=True`, and `extra_content={ORIGINAL_SENDER_KEY: mindroom_user_id(config, runtime_paths)}` when non-None (parity with `hooks/context.py:83-113`, `811-830`). Router sender returns `None` on failure (`bot.py:2045-2076`): persist fingerprint/timestamp ONLY on a non-None event ID so failed sends stay retryable.
9. Stop after `max_pokes_per_scan` successfully DELIVERED pokes; deterministic iteration order.
...
Unit: malformed file skip-and-continue; unassigned/unconfigured skip; quiet gate; direct-busy and team-busy skip; pre-send idle recheck race; schedule suppression incl. `new_thread` exclusion and both failure postures; fingerprint dedup (hidden 6th item change re-arms; unchanged never re-pokes); cooldown ordering; three-successful-sends cap (failed send doesn't count and doesn't persist state); once-per-room schedule query; orchestrator lifecycle (start/reload/stop); env override incl. `0` disables; scheduling helper parsing. Regression tests for scope item 2 as above.
