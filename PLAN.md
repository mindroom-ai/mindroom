...
- **Env override**: `MINDROOM_TODO_POKE_INTERVAL_SECONDS` via `runtime_paths.env_value` (precedent `constants.py:848-856`); `0` disables the worker and enabled values must be at least one second. No YAML config surface.

### 3. Scan algorithm (per tick)

1. Enumerate `storage_root/todo/threads/*/todos.json` in sorted order; skip malformed files with a structured warning and continue, while preserving durable dedup records when a source read fails transiently.
...
4. **Quiet gate**: skip scope unless `now - max(updated_at of that agent's actionable items) >= quiet_seconds` (data already in todos.json; restores "don't barge in right after activity" without activity ledgers), while treating timestamps farther into the future than the normal quiet window as already quiet so clock skew cannot wedge a scope.
...
7. **Dedup + cooldown** in `storage_root/todo/poke_state.json` (same lock/atomic-replace discipline): key = canonical tuple of `(assigned_agent, room_id, normalized_thread_id)` (nested mapping or hashed tuple — no string concatenation of raw IDs); store `last_poked_at` + `last_send_failed_at` + `last_fingerprint` + validated consecutive unchanged-repoke and send-failure counts. Fingerprint = canonical serialization of ALL actionable items for that agent in scope (id/title/priority/depends_on/assigned_agent/updated_at) + thread total/terminal counts. Unchanged fingerprint → at most three one-hour anti-stall retries; failed sends → three immediate retries, then one retry per one-hour backstop until success or a fingerprint change resets the failure count; changed successfully delivered work → only after cooldown and resets the unchanged retry count. All elapsed-period gates treat future skew beyond their window as elapsed, a transient poke-state read failure skips the tick, and corrupt content keeps the existing recovery path.
8. **Send**: native todo poke message ("Todo work is ready" style, list up to 5 items with priorities) from the router into the stored room/thread with exactly one explicit assignee mention, literal non-mention todo titles, `trigger_dispatch=True`, and `extra_content={ORIGINAL_SENDER_KEY: mindroom_user_id(config, runtime_paths)}` when non-None (parity with `hooks/context.py:83-113`, `811-830`). Router sender returns `None` on failure (`bot.py:2045-2076`): record every send outcome in worker memory before durable persistence so a write failure remains isolated and repairable without immediate resend. Deliver at most one successful poke per assigned agent in each scan.
...
Unit: malformed file and invalid-item skip-and-continue; duplicate IDs and invalid boundary timestamps; dependencies on skipped items remain blocked; unassigned, unsafe, or unconfigured assignee skip; quiet gate, future timestamp recovery, and immediate newly-unblocked handoff; direct-busy and team-busy skip; pre-send idle and state recheck races; schedule suppression incl. `new_thread` exclusion and both failure postures; fingerprint dedup (hidden 6th item change re-arms; unchanged work gets at most three backstop retries and change resets the count); cooldown ordering and future-skew recovery; three-send-attempt cap with failed-scope fairness, one successful poke per agent, bounded durable send-failure retries, and hourly capped-failure recovery; per-scope persistence failure recovery; JSON, byte-corrupt, non-finite, boolean, and invalid-counter poke-state recovery; transient source and poke-state read protection; once-per-room schedule query; literal backtick fence rendering; orchestrator adapter availability, concrete callback composition, and lifecycle (start/reload/restart/stop); env override incl. `0` disables, subsecond interval rejection, and invalid values; scheduling helper parsing. Regression tests for scope item 2 as above.
