...
7. **Dedup + cooldown** in `storage_root/todo/poke_state.json` (same lock/atomic-replace discipline): key = canonical tuple of `(assigned_agent, room_id, normalized_thread_id)` (nested mapping or hashed tuple — no string concatenation of raw IDs); store `last_poked_at` + `last_fingerprint` + validated consecutive unchanged-repoke and send-failure counts. Fingerprint = canonical serialization of ALL actionable items for that agent in scope (id/title/priority/depends_on/assigned_agent/updated_at) + thread total/terminal counts. Unchanged fingerprint → at most three one-hour anti-stall retries; failed sends → at most three retries before success or a fingerprint change resets the failure count; changed successfully delivered work → only after cooldown and resets the unchanged retry count. A transient poke-state read failure skips the tick, while corrupt content keeps the existing recovery path.
